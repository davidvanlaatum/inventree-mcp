...
- Use update_stock_item_metadata only for batch, expiry date, packaging, notes, or an HTTP(S) external link. Run dry_run:true, review the complete bound stock state, then provide its exact plan_hash with confirm:true. Use bulk_update_stock_item_metadata for the same field set across 1 to 25 independent stock items in one call; each item gets an independent applied/skipped/failed/ambiguous/unverified outcome, never cross-item atomicity.
...
- Current milestone tools can list metadata, download schema-exposed attachment or part-image content, upload inline or allowlisted local files, upload URL copies, create stored links, update metadata, batch-update metadata across up to 25 existing attachments with bulk_update_attachment_metadata, delete confirmed attachments, set a primary part image from a stable same-part image attachment, and assign, replace, or clear an existing company's primary image.
- bulk_update_attachment_metadata accepts exactly update_attachment_metadata's field set (filename, comment, tags) per item; uploads, links, deletes, and primary-image assignment stay on the single-item tools. Review dry_run:true's planned/failed items, then resend with confirm:true and the returned plan_hash within five minutes from the same principal; each item's applied/skipped/failed/ambiguous/unverified outcome is independent, never atomic.
