...
func TestPinnedRouterEvidenceCoversEveryRoutePattern(t *testing.T) {
	t.Parallel()
	r := require.New(t)
	data, err := os.ReadFile("testdata/inventree-1.5.0-router-routes.txt")
	r.NoError(err)
	evidence := string(data)
	r.Contains(evidence, "source_commit=6a6736b8d6afea24625a67f5f55024cc2f6317b0")
	r.Contains(evidence, "source_blob=6c8596712a1bad558fde8275d8524480c5cdc430")
