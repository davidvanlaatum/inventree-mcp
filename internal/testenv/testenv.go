	...
	if opts.StartWorker {
		workerOpts := []testcontainers.ContainerCustomizer{
			testcontainers.WithConfigModifier(func(config *container.Config) {
				// The worker shares the server network namespace and does not need
				// the image's web port exposed or published on the host.
				config.ExposedPorts = nil
			}),
			testcontainers.WithHostConfigModifier(func(hostConfig *container.HostConfig) {
				hostConfig.NetworkMode = container.NetworkMode("container:" + server.GetContainerID())
			}),
			testcontainers.WithEnv(inventreeContainerEnv(dbHost, cacheHost, secretKey)),
