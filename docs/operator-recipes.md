...
Production reverse-proxy HTTP deployment has configured canonical protected-resource and authorization-server metadata, bearer challenges, token audiences, setup/token endpoints, and OAuth-protected startup wiring for `/mcp`. It still depends on the remaining canonical URL and trusted-proxy enforcement for those routes plus live deployment validation before it should be treated as supported for operators.
...
- Required inputs: internal listen address, `INVENTREE_URL`, public HTTPS issuer/resource URLs, envelope keys, allowed client metadata URLs, and token lifetimes. Trusted proxy settings remain future inputs; setup/token endpoints apply bounded bodies, timeouts, and per-source-address rate limits now.
...
- Clarify when: the public URL differs from proxy routing, path prefix handling is unclear, production config enables TLS skip verify, the requested client ID is not an HTTPS CIMD URL, or the document does not advertise `private_key_jwt` and a same-origin JWKS.
- Expected output now: HTTP MCP endpoint with connector setup, canonical configured OAuth metadata, signed-client token exchange, and no MCP dispatch without a valid encrypted access-token envelope. Trusted-proxy/path-prefix enforcement and live connector validation remain F-S09/F-S10 work.
...
- Remaining gaps: parameter template/default administration and broader live-order-entry recovery surfaces remain tracked separately in F-S11 through F-S15.
