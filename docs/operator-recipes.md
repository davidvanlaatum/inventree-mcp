...
- Required inputs: one of the registered prompt names: `new_part_entry_checklist`, `parameter_reuse_checklist`, `attachment_image_checklist`, `initial_stock_entry_checklist`, `purchase_preview_checklist`, or `receive_purchase_order_checklist`.
...
- Expected output: a stable-ID retry request, a dry-run plan for write-capable workflows, a no-write purchase preview, or a structured clarification object. Future prompt names such as `bom_import_review` and `stocktake_review` are not exposed until their workflows are implemented.
