...
- Expected output: dry-run action summaries plus field-level `planned_changes` for every order and line create or patch, including derived references, quantities, prices, currencies, dates, destinations, notes, and the client-controlled `auto_pricing:false` and `merge_items:false` safety fields. A line's `depends_on` maps its unresolved `order` field to the planned order create when that stable ID does not exist yet. This is an advisory preview rather than a confirmation token; execution reruns preflight and may differ after intervening upstream changes. On execution, expect a stable purchase-order ID, its generated InvenTree reference, stable line IDs, retry-recoverable create/update results, and structured partial-failure output with completed actions and a concrete recovery plan. The exact `(supplier_id, supplier_reference)` pair is the retry identity; InvenTree generates the internal purchase-order `reference`, derived line references use `<supplier_reference>-<one-based index>`, and retries do not delete extra existing lines. If more than one order has the exact pair, the workflow asks which order to reuse. The MCP annotation remains `idempotentHint:false` because concurrent creators can race across the non-atomic lookup/create boundary.
...
- After creation, keep placement and receipt separate: dry-run `issue_purchase_order`, review the current order and every returned line plus the target `PLACED` status in `planned_changes`, then call it with `confirm_issue:true` and the exact returned `plan_hash` only when the order is ready to be placed with the supplier. The confirmation call re-reads and hashes current order/line state, so any metadata, identity, quantity, pricing, destination, date, note, or receipt change observed before placement requires a fresh dry run. InvenTree cannot atomically combine that final preflight with the placement request: coordinate a single writer while issuing, and inspect the current order and lines before retrying any ambiguous result.
