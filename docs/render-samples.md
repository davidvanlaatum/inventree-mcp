...
Example `render_component_image` output for every supported family (F-S90). Generated by `cmd/generate-render-samples` from `internal/render.Samples`; the same list backs `TestRenderSamplesMatchCheckedInGallery`, so these images are checked against what the renderer actually produces (within a small bounded pixel tolerance that accounts for cross-CPU-architecture anti-aliasing rounding), not just illustrative screenshots. Regenerate with `go generate ./internal/render/...` after a rendering change and review the diff before committing.
