...
Tools in this section record status `milestone_1` in the checked manifest. Ordinary write tools use mutation class `write` with MCP annotations `readOnlyHint:false`, `destructiveHint:false`, `idempotentHint:false`, and `openWorldHint:false`. The combined purchase-order workflow remains `idempotentHint:false` because its recovery lookup and create are not atomic. Stock creation and purchase receiving tools use mutation class `operational`. `upload_attachment_from_url` uses `openWorldHint:true`; `delete_attachment` uses mutation class `destructive` with `destructiveHint:true`. Most tools require OAuth scope `inventree.write` when OAuth authorization mode is enabled. Operational stock tools require `inventree.write` and `inventree.operational`; purchase receiving additionally requires `inventree.read` so the same caller can perform mandatory unknown-result recovery. Attachment tools require `inventree.write` and `inventree.upload`; delete also requires `inventree.destructive`.
