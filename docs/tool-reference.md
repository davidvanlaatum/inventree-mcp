...
| `stock_item_id`, `reason`, `confirm`, `plan_hash` | stock adjustment tools | One stable stock-item ID, a required nonblank audit reason, explicit execution confirmation, and the opaque principal-bound five-minute single-use token returned by the latest dry run. A newer dry run for the same action/item supersedes the prior token; outstanding storage is bounded. |
...
Tools in this section record status `milestone_1` in the checked manifest. Ordinary write tools use mutation class `write` with MCP annotations `readOnlyHint:false`, `destructiveHint:false`, and `openWorldHint:false`; most use `idempotentHint:false`. `update_part_category` uses `idempotentHint:true` because it applies absolute PATCH values to one stable ID and reads current state before retry guidance. The combined purchase-order workflow and bulk parameter propagation remain `idempotentHint:false` because writes and recovery reads are not atomic. Stock creation, stock adjustments, and purchase receiving use mutation class `operational`. `upload_attachment_from_url` uses `openWorldHint:true`; attachment/parameter/category-default deletion, parameter-template merge, and bulk parameter propagation use mutation class `destructive` with `destructiveHint:true`. Most tools require OAuth scope `inventree.write` when OAuth authorization mode is enabled. Category, template, and category-default create/update require `inventree.read` because collision, reference, hierarchy, and recovery preflight can disclose existing records. Initial stock creation requires `inventree.write` and `inventree.operational`; current-state-bound stock adjustments and purchase receiving additionally require `inventree.read` for preflight and unknown-result recovery. Attachment tools require `inventree.write` and `inventree.upload`; attachment delete also requires `inventree.destructive`. Parameter-row, unreferenced-template, and category-default deletion, template merge, and bulk parameter propagation require `inventree.read`, `inventree.write`, and `inventree.destructive` so they can preflight references or identities, present current records or plans, verify mutations, and authorize value replacement.
...
| `create_purchase_order_with_lines` | Purchasing workflow | `dry_run`, `supplier_id`, required stable `supplier_reference`, optional `purchase_order_id` ambiguity resolution, optional order metadata, and `lines` with supplier-part identity, quantity, pricing, notes, target date, and destination | `status`, `dry_run`, `supplier_reference`, `purchase_order`, `lines`, `actions`, optional `failure` or `clarification` | Supplier/order/line identity conflicts, preview validation fails, or the exact supplier and supplier-reference pair matches ambiguous remote state. The workflow is retry-recoverable but retains `idempotentHint:false` because duplicate lookup and creation are not atomic. |
| `issue_purchase_order` | Purchasing | `dry_run`, `order_id`, `confirm_issue` | `status`, `dry_run`, `order`, `action`, optional `clarification` | Order is not pending, or placing it with the supplier lacks `confirm_issue:true`. Already-placed orders return `already_placed` without mutation. |
...
| `receive_purchase_order_checklist` | `milestone_1` | Prepare explicit issue and purchase-order receiving workflows. | Use stable IDs and dry runs, keep issue and receipt separate, receive only outstanding quantities, resolve destinations, and require explicit confirmations. |
...
| `create_purchase_order_with_lines` | Purchasing workflow | Write | `inventree.write` | None | `readOnlyHint:false`, `destructiveHint:false`, `idempotentHint:false`, `openWorldHint:false` | `registered_with_oauth_scope_guard` | Retry identity or supplier-part validation is ambiguous. |
| `issue_purchase_order` | Purchasing | Write | `inventree.write` | None | `readOnlyHint:false`, `destructiveHint:false`, `idempotentHint:false`, `openWorldHint:false` | `registered_with_oauth_scope_guard` | Order is not pending or `confirm_issue:true` is missing. |
