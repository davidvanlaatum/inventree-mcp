...
Tools in this section record status `milestone_1` in the checked manifest. Ordinary write tools use mutation class `write` with MCP annotations `readOnlyHint:false`, `destructiveHint:false`, `idempotentHint:false`, and `openWorldHint:false`. The combined purchase-order workflow remains `idempotentHint:false` because its recovery lookup and create are not atomic. Stock creation, stock adjustments, and purchase receiving use mutation class `operational`. `upload_attachment_from_url` uses `openWorldHint:true`; attachment/parameter deletion and parameter-template merge use mutation class `destructive` with `destructiveHint:true`. Most tools require OAuth scope `inventree.write` when OAuth authorization mode is enabled. Template create/update additionally require `inventree.read` because collision preflight reads and may disclose existing templates. Initial stock creation requires `inventree.write` and `inventree.operational`; current-state-bound stock adjustments and purchase receiving additionally require `inventree.read` for preflight and unknown-result recovery. Attachment tools require `inventree.write` and `inventree.upload`; attachment delete also requires `inventree.destructive`. Parameter-row deletion, unreferenced-template deletion, and template merge require `inventree.read`, `inventree.write`, and `inventree.destructive` so they can preflight references, present a current plan, and verify mutations.
