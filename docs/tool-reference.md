...
All tools in this section are implemented and registered. Milestone 1 lookups use status `milestone_1`; the F-S03 purchasing recovery lookups remain `future` until the full story is complete. They use class `read_only`, MCP annotations `readOnlyHint:true`, `destructiveHint:false`, `idempotentHint:true`, and `openWorldHint:false`. They require OAuth scope `inventree.read` when OAuth authorization mode is enabled. Upload sources are `None`.
...
Tools in this section record their milestone status in the checked manifest. Milestone 1 tools remain `milestone_1`; the registered F-S03 purchasing slice remains `future` until the full story, including the product-gated receiving decision, is complete. Ordinary write tools use mutation class `write` with MCP annotations `readOnlyHint:false`, `destructiveHint:false`, `idempotentHint:false`, and `openWorldHint:false`. The combined purchase-order workflow remains `idempotentHint:false` because its recovery lookup and create are not atomic. Stock creation tools use mutation class `operational`. `upload_attachment_from_url` uses `openWorldHint:true`; `delete_attachment` uses mutation class `destructive` with `destructiveHint:true`. Most tools require OAuth scope `inventree.write` when OAuth authorization mode is enabled. Operational stock tools require both `inventree.write` and `inventree.operational`. Attachment tools require `inventree.write` and `inventree.upload`; delete also requires `inventree.destructive`.
...
Post-milestone purchasing and live order-entry work is tracked in `docs/TASKS.md`. The registered F-S03 creation slice exposes `create_purchase_order_with_lines` with preview-equivalent validation, exact supplier plus supplier-reference retry identity, purchase-order and line read/search support for duplicate checks and recovery, and structured failure output that includes any created IDs. Receiving remains unavailable pending its product decision. F-S15 should also make lower-level write tools consistently support dry-run/preflight where practical, reject blank/null MPN before manufacturer-part writes or document an operator-approved fallback convention, and include redacted InvenTree response-body details in tool errors.
...
- Remaining purchase-order tools: `receive_purchase_order_items`, `close_purchase_order`. Receiving remains product-gated until stock create/merge/update semantics are approved.
