...
| `filename`, `content_type`, `comment`, `tags` | attachment write tools | Optional or required attachment metadata, preserving explicit empty values on metadata updates. |
...
| `create_link_attachment` | Attachments | `model_type`, `model_id`, `url`, optional `filename`, `comment`, `tags`, `allow_duplicate` | `status`, `record`, `source_kind`, optional `clarification` with retry `model_id` or `allow_duplicate` | URL is not HTTP(S), includes credentials or a fragment, target object is out of scope, or duplicate preflight matches an existing link. |
