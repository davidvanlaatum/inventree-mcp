...
`inventree-mcp version` prints human-readable text by default: a single `inventree-mcp <version> (commit <commit>, built <date>)` line, plus an `InvenTree baseline: <version> (API <api>)` line whenever the build carries `internal/buildinfo`'s baseline constants.

`inventree-mcp version --porcelain <N>` instead prints a versioned, key-based porcelain-style machine format for porcelain format version `N`: an explicit `porcelain: 1` marker followed by `key: value` lines for `version`, `commit`, and `date`, plus the optional `inventree_version`/`inventree_api` InvenTree baseline whenever the build carries those constants. Parsers split each line on the first `": "` occurrence — not a fixed line count and not a naive split on every colon, since `date` is RFC3339 and itself contains colons — and look up known keys by name, tolerating any additional keys a later revision adds. `--porcelain` requires an explicit format-version argument (currently only `1` is supported); the command fails closed with a clear error and no stdout output when asked for a format version it does not know how to produce. Only an incompatible change to an existing key's meaning requires bumping that format version.

`self-update`'s safety check (`requireVersion`) runs `version --porcelain <N>` against the staged candidate before replacing the installed executable, always requesting the porcelain format version this build itself understands (`internal/buildinfo.PorcelainMarker`) rather than the target release version, decoupling format negotiation from release-version verification. It requires the response's own `porcelain` marker to also be a version it understands and requires the reported `version` field to match the target release, failing closed otherwise. Requesting the format version establishes the negotiation *mechanism* for a future format change; today's binaries only ever serve their own single current format version on request, so a future format bump still breaks self-update from an older updater exactly like this story's own migration, unless a later release adds logic to serve an older format on request. It reuses that same pre-rename parse — not a second execution of the candidate — to report the old-to-new InvenTree baseline alongside the old-to-new MCP version in `self-update`'s result summary, omitting the baseline line when the InvenTree fields are legitimately absent from an otherwise-valid response.

This porcelain format replaced a fixed 3-line `version`/`commit`/`date` output as a deliberate, one-time breaking change, accepted while the operator is the only current user. Every binary released before that change shipped (`v0.0.1`-`v0.0.11` and any that followed before it merged) has a compiled-in `requireVersion` that still invokes bare `version` (with no `--porcelain` flag) and expects the old exact-3-line shape in response. The candidate's bare `version` now prints the new human-readable text instead, which the old parser also rejects as malformed, so that binary refuses to self-update into the first release containing this change. That specific release must be installed manually: download its release archive from the repository's GitHub Releases page (see README's [Install From A Release](../README.md#install-from-a-release) section for the archive layout), verify it against the published `checksums.txt`, replace the executable, then run `inventree-mcp version` to confirm it reports the new format. Every release after it self-updates normally on the new versioned contract. This must not recur casually after `v1.0.0` — the same porcelain format-version negotiation should be used for any future breaking change instead of repeating an undocumented break.
...
5. Re-runs the bounded `version --porcelain <N>` command through the installed path.
