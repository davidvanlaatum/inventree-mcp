...
- `internal/inventree.PartFieldInventory` classifies every pinned API 530 `Part` property. Scalar identity, category/default IDs, flags, image/thumbnail, IPN/revision/units/notes, creation metadata, pricing bounds, and nullable stock/allocation aggregates are exposed. Nested category/default-location detail, category path, and parameters are separate lookups. Tags, price breaks, ownership, and the not-yet-exposed revision-family fields are deferred; write-only duplication/initialization/image inputs remain unavailable; raw `barcode_hash` is excluded.
