...
The checked REST schema exposes stable primary keys and endpoint/media/link fields, but it does not define a canonical user-facing frontend URL for each object. F-S34 and its F-S37 default-mount correction therefore treat frontend construction as a separate version-pinned MCP contract: browser `web_url` values use exact optional `INVENTREE_WEB_URL`, or all-mode fallback to `INVENTREE_URL` plus the stock `/web` mount, after the same credential-free validation. The reviewed InvenTree 1.4.3 evidence pins commit `6b237de54e4cbfd7f51daff8403c17869898d965`, nested router blob `ddeb3a21365761e999568c84d6417915817a9024`, `BrowserRouter` mount blob `ba921811446397503ef4d45f5476e504888882ef`, and default-base helper blob `939dd0211522e4ebcb06cbf0d3b587ae3dbf2721`; browser links remain distinct from REST and media URLs. Records without their own stable frontend page omit `web_url` and use universal `parent_web_url` only for an immediate owning object with a stable frontend page and identity present in the same projection. The complete matrix and output inventory are in [`docs/web-links.md`](web-links.md).
