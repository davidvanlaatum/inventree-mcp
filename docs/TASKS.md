...
- Residual risk: generation is a non-atomic inventory snapshot and may change while processing. The guarded tools fail closed on missing IDs, task errors, identity mismatch, unsafe report URLs, redirects, and oversized content; `pending` requires the agent to retain and reuse the task ID. Duplicate/same-day, non-staff permission, and bounded terminal/report behavior are characterized, but the pinned combined report task did not complete or expose an artifact within 30 seconds; report production remains an upstream operational dependency. Formal reviewer coverage remains pending.
