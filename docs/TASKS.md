...
- Acceptance:
  ...
  - A route matrix covers default and non-default prefixes, trailing-slash normalization, nearby paths such as `/mcpfoo`, metrics collisions, metadata paths, exact `/prefix` and `/prefix/*` proxy behavior, and confirms root-level `/authorize` and `/token` do not reach the new handlers or InvenTree; no compatibility alias is required.
...
- Progress: identified during live packaged HTTP use after OTEL traces were confirmed working. Current tool logs are DEBUG-only and carry the noisy `source_ip_forwarded` provenance flag; the packaged YAML template includes Prometheus metric examples but omits the equivalent OTEL tracing sample. A configuration audit also found that the HTTP template omits examples for `inventree_auth_scheme`, `inventree_timeout`, `inventree_tls_skip_verify`, `mcp_max_request_body_bytes`, `debug_traffic_log`, `bulk_max_items`, and `bulk_concurrency`. The upstream token and STDIO upload-root settings are intentionally transport-specific omissions for the HTTP template and must be documented as such rather than copied blindly.
- Decisions: tool invocation and completion records will be INFO-level structured logs, independently of whether OTEL tracing or metrics are enabled. Establish the request-scoped logger with `dvgoutils/logging.WithLogger` at the HTTP/tool boundary, embedding stable request, transport, tool, source-IP, and valid-span trace fields once; downstream handlers and the InvenTree client use `logging.FromContext` and derive child loggers with `logger.With(...)` rather than receiving logger parameters through every function. Keep normalized `source_ip` but omit `source_ip_forwarded`, while preserving trusted-proxy source resolution. Use a centralized deny-by-default field builder with an explicit per-tool field map, fixed types/counts/lengths, and no arbitrary `*_id` inference. Search logs omit raw search text and presence flags, and may include only bounded numeric parameters or allowlisted filter IDs. Trace IDs come only from a valid current `SpanContext`, are fixed-width correlation values rather than identity/authentication, and are omitted for absent/invalid/zero contexts. Tool completion logs include outcome, duration, and bounded result counts where available. Outbound InvenTree request completion logs use explicit `inventree_request_started`/`inventree_request_completed` events and a request/tool-call ID plus call sequence and attempt, making multi-call tools diagnosable without logging request bodies, credentials, raw query values, full URLs, or unbounded record data; they derive the request logger from context and add method, fixed normalized API operation/route template, status or closed safe error class, duration, and retry/attempt where applicable. The outbound boundary includes JSON, multipart, attachment/image/data-output downloads, and other configured InvenTree API calls, but excludes OAuth, bootstrap, and arbitrary URL-fetch traffic. Packaged tracing configuration remains disabled by default and mirrors the documented OTEL settings.
- Scope: update independent request/tool and outbound InvenTree API logging, centralized safe tool/API context extraction, completion/outcome/duration logging, trace correlation, deterministic clock/tracer seams, redaction tests, packaged YAML samples, and relevant README/operator/reference documentation.
- Acceptance:
  ...
  - Outbound InvenTree request completion logs identify the originating tool, method, fixed normalized API operation/route template, status or closed safe error class, duration, request/tool-call ID, call sequence, and retry/attempt when applicable, allowing multi-call tool sequences to be reconstructed without request bodies, auth headers, raw query values, full URLs, or unbounded record data; stable request/tool/trace fields are inherited from the context-carried logger.
  - Valid fixed-width `trace_id`/`span_id` values are included only for a valid current span, with no bogus IDs when tracing is absent, invalid, or zero; raw trace headers are never logged.
  - Tests cover context-carried logger inheritance/enrichment, log level with telemetry disabled, source-IP field removal and trusted/untrusted forwarding, the centralized field map and bounds, representative ID/search/bulk context, tool and outbound-request completion fields, multi-call correlation, route/error redaction, deterministic clock/tracer behavior, and trace correlation.
  - `packaging/inventree-mcp.yml` includes the complete applicable HTTP configuration examples, including commented OTEL tracing settings matching `docs/examples/inventree-mcp.yml`, with safe guidance for collector headers and other secret-bearing configuration. The `debug_traffic_log` example warns that it captures sensitive URIs, bodies, responses, and possible credentials and is for owner-only troubleshooting, not routine production use. Transport-specific omissions such as the raw upstream token and STDIO local-upload roots are explicitly explained; HTTP-inapplicable settings are not presented as usable HTTP options.
  - README, operator recipes, and relevant reference docs remain aligned; validation names the default and race test suites, vet, lint, package/config loader checks, GoReleaser snapshot, per-package coverage comparison, and the required Go, QA, Product, and Infosec review panel with no unresolved findings.
...
- Tasks:
  ...
  - [ ] Implement context-carried INFO invocation/completion logs, outbound InvenTree request completion logs, and trace correlation.
  - [ ] Add focused tool/API context, completion, multi-call, trace, and redaction tests; update documentation.
