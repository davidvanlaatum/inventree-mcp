...
- Validation: `go test ./...` passed with workspace-local Go build cache before commit. Follow-up CI alignment with `dvgoutils` added Go coverage reporting after `go test -coverpkg=./... -coverprofile=/private/tmp/inventree-mcp-coverage.out ./...` passed and reported 82.6% total coverage. Coverage badge follow-up passed YAML load validation for CI configs, the same cached Go test and coverage commands, and `git diff --check`. Gremlins cache follow-up persisted the Gremlins job's Go build and module caches and passed GitHub workflow YAML load validation, `go run github.com/rhysd/actionlint/cmd/actionlint@latest .github/workflows/go.yml`, and `git diff --check`; PR #21 CI then passed lint, test, and Gremlins, with the first Gremlins run reporting a cache miss and saving a 570 MB cache for the Gremlins Go-cache key.
