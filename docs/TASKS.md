...
- [x] Rewrite `internal/selfupdate.requireVersion` to return parsed fields alongside its pass/fail result, widen `installVerified`'s return type, and add the new `Result` fields.
