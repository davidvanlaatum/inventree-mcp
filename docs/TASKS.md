...
- Acceptance:
  ...
  - A route matrix covers default and non-default prefixes, trailing-slash normalization, nearby paths such as `/mcpfoo`, metrics collisions, the exact unchanged metadata locations `/.well-known/oauth-protected-resource` and `/.well-known/oauth-authorization-server`, arbitrary `/.well-known/*` negative cases, exact `/prefix` and `/prefix/*` proxy behavior, and confirms root-level `/authorize` and `/token` return an explicit non-InvenTree rejection (normally 404/410), do not reach the new handlers or InvenTree, and have no compatibility alias.
...
- Acceptance:
  ...
  - Tests cover context-carried logger inheritance/enrichment, log level with telemetry disabled, direct/trusted/untrusted/malformed source-IP forwarding and STDIO behavior, the centralized field map and bounds, representative ID/search/bulk context, tool invocation/completion and outbound-request started/completed fields, two API calls in one invocation, separate invocations using the same route, bounded correlation reset/uniqueness, route/error redaction, exact duration with an injectable clock, isolated tracer-provider behavior, and trace correlation for absent/zero/invalid/remote-parent/local-child spans.
  - `packaging/inventree-mcp.yml` includes the complete applicable HTTP configuration examples, including commented OTEL tracing settings matching `docs/examples/inventree-mcp.yml`, with safe guidance for collector headers and other secret-bearing configuration. A raw-template inventory test asserts the expected active/commented/forbidden state for every audited setting, including the explicitly HTTP-inapplicable `inventree_auth_scheme`. The `debug_traffic_log` example warns that it captures sensitive URIs, bodies, responses, and possible credentials, is owner-only (0600), requires bounded retention/deletion, and is for troubleshooting rather than routine production use. Every audited setting is classified as valid for production HTTP, documented but rejected outside development, or transport-specific and omitted; HTTP-inapplicable settings are not presented as usable HTTP options, and omissions such as the raw upstream token and STDIO local-upload roots are explicitly explained.
  - README, operator recipes, and relevant reference docs remain aligned; validation runs `go test ./...`, `go test ./... -race`, `go vet ./...`, `golangci-lint run ./...`, package/config-template checks, `goreleaser release --snapshot --clean`, per-package coverage comparison, and the required Go, QA, Product, and Infosec review panel with no unresolved findings.
...
- Tasks:
  ...
  - [ ] Implement context-carried INFO invocation/completion logs, outbound InvenTree request completion logs, request classification, and trace correlation.
