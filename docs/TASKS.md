...
- Acceptance:
  ...
  - Deletes only after `confirm:true` on an already-reviewed clean part, then verifies exact-ID absence via read-back before reporting success.
  - An ambiguous read-back after a confirmed delete call returns `partial_failure` with a read-before-retry recovery plan instead of a false success or silent data loss.
  - Requires `inventree.destructive` scope and publishes `destructiveHint:true`, `idempotentHint:false` MCP annotations.
  - Unit and default-on pinned InvenTree 1.4.3 Testcontainers coverage exercise invalid ID, not-found, each individual blocking category (including active-state), preview-then-confirm deletion, validation failure, and ambiguous read-back.
