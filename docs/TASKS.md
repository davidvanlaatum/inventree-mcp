...
| [F-S33](#f-s33-guarded-part-deletion) | Add guarded deletion of one unreferenced ordinary part, refusing while the part is active or stock, BOM, build, purchase-order, sales-order, variant, supplier/manufacturer-part, parameter, attachment, or related-part references exist. | Active |
...
- Review: none yet; requesting Senior Go Developer and Senior QA / Test Architect review before merge because this adds a new destructive mutating tool surface with a safety guard that diverges from upstream InvenTree's own permissiveness, and because it fixes a pre-existing shared-test-infrastructure bug (`internal/testenv`) used by every other integration-tested task.
...
- [x] Classify `DeletePart` mutation errors as definite-rejection vs. ambiguous (matching `deplete_stock_item`/`transfer_stock_item`'s `definiteStockMutationRejection` pattern), verify ambiguous failures by read-back instead of reporting them as clean failures, and preserve `context.Canceled`/`context.DeadlineExceeded` through every preflight and mutation error path instead of masking them behind a generic message.

- Validation: `go build ./...`, `go vet ./...`, `gofmt -l .`, `golangci-lint run ./...` (0 issues), `go mod tidy -diff` (clean). `go test ./...` passes, including the default-on Docker-backed InvenTree 1.4.3 integration suite (`internal/inventree`, `internal/testenv`, `internal/tools`, `internal/schema`). `TestClientMethodsAgainstInvenTree/part_delete` pins, per category in isolation on an inactive part, that InvenTree 1.4.3 permits deleting a part with existing stock (destroying the stock item), its own BOM, a build, a purchase-order line (orphaned, not destroyed), a sales-order line, a variant, a supplier part, a manufacturer part, a parameter, or an attachment, and rejects only an active part or one used as a component in another part's BOM. New unit tests (`TestDeletePartRecoversWhenAmbiguousMutationErrorAppliedAnyway`, `TestDeletePartPartialFailureWhenAmbiguousMutationErrorDidNotApply`, `TestDeletePartReturnsSafeErrorOnDefiniteMutationRejection`, `TestDeletePartTreatsRetryableStatusAsAmbiguous`) cover the corrected mutation-error classification, including the 408/425/429 retryable-status boundary. `go generate ./internal/tools` produced the checked manifest with no diff.
- Review: Round 1 (Senior Go Developer, Senior QA/Test Architect, Senior Product Manager, Senior Infosec Reviewer, run against the initial "informational-only" design) found: an inefficient N+1 purchase-order-line query fan-out (fixed with a dedicated `base_part` filter query), missing integration coverage for the confirmed-delete success path, a missing `variant_of` blocking check, an uncommitted `docs/TASKS.md` audit-trail gap, and a missing operator-recipes entry. Fixing the "confirmed-delete success path" gap led directly to the informational-vs-blocking and active-state discoveries above, which required redesigning the guard. Round 2 (focused Go/QA/Product/Infosec re-review of the corrected design) found the guard logic, `PartDeleteBlockingReferences` wiring, and unit-test coverage were correct, but caught two integration-test/unit-test comment blocks and this story's own Scope/Acceptance text that still described the superseded "informational, not blocking" design (fixed to match the shipped, evidence-based behavior), a stale `PartRelation` model doc comment, and an unverified assumption that InvenTree's `?part=` related-part filter matches either side of the relation (verified by pinning the part-under-test as `part_2` rather than only `part_1`). Round 3 was the operator's own PR review (2026-08-09, head `b9fd463`) and found the most significant issue of any round: `deletePart` returned immediately on any non-validation `DeletePart` mutation error without verifying whether the deletion had actually applied, so a lost/ambiguous response after InvenTree processed the delete could be misreported as a clean failure -- contradicting both issue #104's requested recovery behavior and this story's own `partial_failure` claim. Fixed per the finding above. The operator also flagged that `context.Canceled`/`context.DeadlineExceeded` were being masked by the generic `safePartDeleteError` wrapper (unlike the newer `stock_transfer_tools.go`/`stock_depletion_tools.go` mutation paths), that the confirmation contract's "already-reviewed" wording overclaimed what the handler proves, and several alignment items (this story's status vs. the issue's status context, the unreviewed F-S32 dependency, a missing `docs/api-schema.md` update, and the first commit's missing `Task:` trailer). All addressed except the unreviewed F-S32 dependency, which is not this story's to resolve and is flagged above instead of silently ignored. Findings from all three rounds were addressed; a further focused re-review of this round's changes is expected before this story moves to `Done`.
- Residual risk: preflight checks and the delete call are separate InvenTree requests, so a concurrent writer could create a new reference between the guard check and the delete call; operators should coordinate a single writer, matching the same accepted risk as F-S32. No MCP tool currently exists to remove a supplier-part, manufacturer-part, or related-part link (unlike parameters via `delete_part_parameter` and attachments via `delete_attachment`), so an operator blocked by one of those three categories must clear it through the InvenTree UI directly before retrying `delete_part`. Only stock, the part's own BOM, purchase-order lines, and supplier/manufacturer/related-part links were individually verified for their post-delete survival/destruction behavior; builds, sales-order lines, and variants were confirmed to permit deletion but their child-record fate after that was not independently checked -- delete_part refuses on all of them regardless, so this does not weaken the guard, only the completeness of the documented evidence. Confirmation-contract decision, made explicitly rather than left implicit: `delete_part` uses the same stable-ID-plus-fresh-preflight `confirm:true` convention as `delete_purchase_order_line`, `delete_part_parameter`, `delete_purchase_order_extra_line`, and `delete_attachment`, not a state-derived plan hash/token like the stock-adjustment tools (F-S22). This is a deliberate choice, not an oversight: every call, confirmed or not, re-runs the complete blocking-reference preflight fresh, so the actual safety property is "re-verified immediately before deletion," not "provably matches an operator's earlier preview." A plan-hash mechanism would be more precise but is a heavier pattern this codebase currently reserves for stock-quantity-adjustment tools where a plan's calculated values (not just a boolean confirmation gate) need explicit reuse; introducing it for `delete_part` alone, while four sibling delete tools keep the simpler convention, was judged to be inconsistent scope creep rather than a safety improvement proportionate to the risk. `docs/api-schema.md` now records the newly pinned `DELETE /api/part/{id}/` active-state/BOM-component behavior and the guard-only BOM/build/sales-order-line/related-part/variant reads under "Verified Part Deletion And Guard-Only Reads".
