...
| [F-S93](#f-s93-stateless-per-user-bearer-bootstrap-for-http-clients) | Add a stateless, per-user bearer bootstrap flow for HTTP MCP clients that cannot perform OAuth, without storing a server-side token mapping. | Active |
