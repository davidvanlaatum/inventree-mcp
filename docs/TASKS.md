...
- Decisions: tool invocation and completion records will be INFO-level structured logs. Establish the request-scoped logger with `dvgoutils/logging.WithLogger` at the HTTP/tool boundary, embedding stable request, transport, tool, source-IP, and trace fields once; downstream handlers and the InvenTree client use `logging.FromContext` and derive child loggers with `logger.With(...)` rather than receiving logger parameters through every function. Keep normalized `source_ip` but omit `source_ip_forwarded`. Include only bounded, allowlisted tool context fields such as stable record IDs and safe control values; never log arbitrary arguments. Search logs omit raw search text and presence flags, and may include only bounded numeric parameters or filter IDs. When a valid tracing span exists, include `trace_id` and `span_id`; omit them otherwise. Completion logs include outcome, duration, and bounded result counts where available. Outbound InvenTree request completion logs will make multi-call tools diagnosable without logging request bodies, credentials, raw query values, full URLs, or unbounded record data; they derive the request logger from context and add method, normalized API route/resource, status or safe error class, duration, and retry/attempt where applicable. Packaged tracing configuration remains disabled by default and mirrors the documented OTEL settings.
- Scope: update tool and outbound InvenTree request logging, safe tool/API context extraction, completion/outcome/duration logging, trace correlation, redaction tests, packaged YAML samples, and relevant README/operator/reference documentation.
- Acceptance:
  ...
  - Outbound InvenTree request completion logs identify the originating tool, method, normalized route/resource, status or safe error class, duration, and retry/attempt when applicable, allowing multi-call tool sequences to be reconstructed without request bodies, auth headers, raw query values, full URLs, or unbounded record data; stable request/tool/trace fields are inherited from the context-carried logger.
  - Valid `trace_id`/`span_id` values are included when a tracing span exists, with no bogus IDs when tracing is absent.
  - Tests cover context-carried logger fields, log level, source-IP field removal, representative ID/search/bulk context, tool and outbound-request completion fields, multi-call correlation, trace correlation, and redaction.
  - `packaging/inventree-mcp.yml` includes the complete applicable HTTP configuration examples, including commented OTEL tracing settings matching `docs/examples/inventree-mcp.yml`, with safe guidance for collector headers and other secret-bearing configuration. Transport-specific omissions such as the raw upstream token and STDIO local-upload roots are explicitly explained.
  - README, operator recipes, and relevant reference docs remain aligned; full validation and the applicable Go, QA, Product, and Infosec review panel pass.
...
- Tasks:
  ...
  - [ ] Audit and complete the packaged HTTP configuration examples, add the OTEL tracing sample, and validate the template against the real YAML config loader.
  - [ ] Run full validation and the applicable reviewer panel; record residual risk before marking the story Done.
