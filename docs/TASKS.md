...
| [F-S56](#f-s56-cross-object-tag-workflow-discovery) | Investigate tag discovery and consistent assignment/removal across supported object types. | Done |
...
| [F-S90](#f-s90-deterministic-component-image-rendering) | Render deterministic PNG images for common, highly repetitive electronic components. | Done |
| [F-S91](#f-s91-cross-object-tag-search-and-assignment) | Add a cross-object tag search tool and tags fields on the object types with existing MCP tool coverage. | Done |
| [F-S92](#f-s92-render-component-image-family-enum-and-anti-invention-wording) | Expose every closed-vocabulary field on `render_component_image` as a discoverable JSON Schema enum and strengthen anti-invention wording on its optional color/dimension/marking/rating fields. | Done |
...
| [F-S94](#f-s94-prefix-oauth-endpoints-under-the-configured-mcp-path) | Move OAuth authorization and token endpoints under the configured MCP path so they cannot collide with future InvenTree routes. | Ready |
| [F-S95](#f-s95-structured-tool-logging-and-packaged-otel-samples) | Improve structured tool invocation/completion logging with safe context and trace correlation, and add packaged OTEL tracing samples. | Ready |
...
- Decisions: approved by the operator on 2026-08-15 as a separate future cross-object story. Tags are opt-in/shared taxonomy data and are not embedded into ordinary exact reads until their lifecycle is defined. Following the 2026-08-28 discovery findings, the operator approved this implementation contract: (1) cover only the tagged object types that already have MCP `get_*`/`update_*` tools today (`Part`, `Company`, `StockLocation`, `StockItem`, `SupplierPart`, `ManufacturerPart`, `PurchaseOrder`); `Build`, `SalesOrder`, `SalesOrderShipment`, `ReturnOrder`, and `TransferOrder` have no MCP tool surface at all yet and stay deferred until those object families get tools of their own; (2) expose tag assignment as an ordinary `tags` field (whole-array replace, explicit `[]` to clear) on each covered object's existing `update_*` tool, matching the pattern already shipped for `Attachment` tags, rather than a separate confirm-gated workflow — InvenTree's own schema does not gate object-level tag writes behind any special permission beyond ordinary object write access; (3) add a new read-only cross-object `search_tags` tool (optional `model_type` filter, name search) backed by `/api/tag/`, mirroring F-S64's `search_object_parameters` pattern, so agents can discover the shared tag vocabulary and avoid near-duplicate tags before writing. Direct `/api/tag/` entity administration (rename/delete a `Tag` row) stays out of MCP scope: it is staff-only (`a:staff`) in the pinned schema, orthogonal to ordinary object ownership, and not needed for object-level assignment. The concrete tool-surface implementation (covered objects' `get_*`/`update_*` changes, the new `search_tags` tool, tests, and docs) is tracked as a separate follow-up story, [F-S91](#f-s91-cross-object-tag-search-and-assignment), so this discovery story can close on its own approved proposal per its own acceptance criteria.
- Progress: implementation started on `claude/f-s56-cross-object-tags` from `origin/main` at `7054660` on 2026-08-28. Updating issue #137's status label/assignment through the GitHub MCP integration failed with `403 Resource not accessible by integration`; per this repo's status-label policy that transition was stopped rather than worked around at the time, and the `gh` CLI (broader `repo`-scoped token) was used instead to set the `status:done` label and assign the operator once the PR was ready. Added a `cross_object_tag_workflow_discovery` subtest to `internal/inventree/client_methods_integration_test.go` and ran it live against pinned InvenTree `1.5.2`. Opened as draft PR [#243](https://github.com/davidvanlaatum/inventree-mcp/pull/243).
- Discovery findings (2026-08-28): the pinned schema exposes a dedicated shared `/api/tag/` resource (`Tag{pk, name, slug}`, `slug` read-only and lowercase-derived) alongside a `tags: string[]` property declared on `Attachment` (already implemented separately), `Build`, `Company`, `Location` (`stock.stocklocation`), `ManufacturerPart`, `Part`, `PurchaseOrder`, `ReturnOrder`, `SalesOrder`, `SalesOrderShipment`, `StockItem`, `SupplierPart`, and `TransferOrder`. Live evidence against Part, Company, and StockLocation confirms: (1) tags are a single global shared taxonomy, not per-model-type duplicates — the same tag name assigned to a part, a stock location, and a company resolves to one shared `Tag` row (`pk`), and `/api/tag/?model_type=<type>` scopes to "tags currently referenced by that type" rather than creating separate per-type rows; (2) tag-name matching on write is case-insensitive and reuses the existing row's original stored casing rather than creating a sibling or renaming it; (3) a plain `GET` on `/api/part/{id}/`, `/api/company/{id}/`, and `/api/stock/location/{id}/` omits the `tags` field entirely — only the undocumented `?tags=true` query flag includes it, on all three tested types; (4) `tags__name`/`tags__slug` list filters work as tag-based selection filters (confirmed for `/api/part/`); (5) the separately-declared boolean `tags` list filter on `/api/part/` (default `false`) is not a working has-tags presence filter — both `tags=true` and `tags=false` excluded every part, including a known-tagged one, while an unfiltered list found it; (6) an orphaned `Tag` row (every object reference removed) remains readable indefinitely — InvenTree does not auto-delete unused tags; (7) direct `/api/tag/` mutation (`tag_create`/`tag_update`/`tag_destroy`) is staff-only (oauth2 `a:staff`) and a live non-staff account was rejected with HTTP 403 on both create and delete, while the same non-staff-capable object-level `tags` field write (PATCH `tags:[...]` on the tagged object itself) carries no such staff gate in the schema — ordinary ownership of the underlying object is what governs tag assignment, not a separate tag-CRUD permission. `/api/tag/` additionally requires an explicit `limit` query parameter to return its normal paginated `{count, results}` shape; omitting it returns a bare JSON array.
...
- Validation: `go build ./...`, `go vet ./...`, `golangci-lint run ./...` (0 issues), `gofmt -l` clean, and `git diff --check` all pass. `go test -tags no_integration_tests ./...` passes across every package. `GOFLAGS=-trimpath go test -race -p=1 ./internal/inventree -run '^TestClientMethodsAgainstInvenTree$/cross_object_tag_workflow_discovery$' -count=1 -v` passes live against pinned InvenTree `1.5.2`. A full `GOFLAGS=-trimpath go test -race -p=1 ./...` run hit an unrelated transient failure in the pre-existing `TestClientMethodsAgainstInvenTree/part_delete` subtest (a PostgreSQL foreign-key-constraint error deleting a part still referenced by `PartPricing`, consistent with this repo's documented history of Testcontainers CI-contention flakiness per F-S83/F-S89); `part_delete` passed cleanly in isolation and a full-suite rerun afterward passed clean with no failures, confirming this story's change was not the cause. No package touched by this story has any production-code change, so no coverage comparison applies.
- Review: Senior Go Developer and Senior QA / Test Architect subagent review completed (minimum panel for a Go + test-only change per `docs/reviewers.md`; the full four-role panel was not run because this story adds no production code, tool surface, or auth/infosec-relevant behavior — only a new live-evidence integration subtest and documentation). QA found the "plain GET omits `tags`" and "`?tags=true` includes `tags`" characterizations were only backed by real assertions for `Part`; the parallel `Company`/`StockLocation` checks were logged but not asserted, contradicting this section's Discovery findings claim that all three types were "confirmed." Fixed by converting both checks to `assert.False`/`assert.ElementsMatch` for `Company` and `StockLocation`, matching the existing `Part` assertions, and re-validated live. Go reviewer found no blocking issues; two non-blocking observations are accepted as-is: the non-staff-account setup block duplicates an existing block in the same file's `stocktake` `generation_` subtest (worth extracting to a shared helper next time either is touched, e.g. by F-S91, but not before) and the `patchTags` test helper builds raw `NewRequest`/`DoJSON` calls instead of the existing `Client.Patch` convenience wrapper (harmless here since it deliberately decodes into an untyped map — no `Tags` field exists on any typed model yet, by design for a discovery story). Both reviewers separately confirmed the new `rawPartIDs` helper was correctly named to avoid colliding with the file's existing differently-typed `partIDs` helper, and that `fixture.run.Name(...)`-scoped tag/fixture names carry no test-pollution risk in the shared parallel Testcontainers suite.
- Residual risk: tag plugins, permissions, and shared-taxonomy changes can affect many objects; a cross-object design must avoid silently creating or deleting global tags. This story adds no production code, so these risks apply to F-S91's implementation, not to this story's own change. The duplicated non-staff-account test boilerplate (see Review) should be extracted into a shared helper by F-S91 rather than copied a third time.
...
- InvenTree 1.5.2 maintenance validation: `go build ./...`, `go vet ./...`, `golangci-lint run ./...` (0 issues), `go generate ./internal/tools` (no manifest drift), `GOFLAGS=-trimpath go mod tidy -diff`, `gofmt -l .` (clean), and `git diff --check` pass. `GOFLAGS=-trimpath go test -race ./internal/testenv -run '^TestStartInvenTreeStack$' -count=1 -v` passed against a freshly pulled `inventree/inventree:1.5.2` and confirmed runtime InvenTree `1.5.2` / API `530`. `GOFLAGS=-trimpath go test -race -p=1 ./...` passed with all default-on Docker suites against the same pin, including the `internal/schema` cross-package contract test and `readme_compat_test.go`'s README-row/pin equality check. After a review-requested fix to `internal/testenv/testenv_test.go` (see Review), `GOFLAGS=-trimpath go test -race ./internal/testenv/... ./internal/buildinfo/... -count=1 -cover` was rerun and passed with coverage unchanged from the pre-change `main` baseline (`internal/testenv` 81.3%→81.3%, `internal/buildinfo` 100.0%→100.0%, measured by stashing the diff, running the same command against clean `main`, then restoring it); no other package's production code changed, so no further per-package coverage comparison was needed.
- InvenTree 1.5.2 maintenance review: Senior Go Developer, Senior QA / Test Architect, Senior Product Manager, and Senior Infosec Reviewer reviews run. Go found no blocking issues, confirmed the drift-risk sweep is clean (every `Default*`/`Pinned*` consumer stays derived from the constants, and the 1.5.1 review's `testenv_test.go:974` fix was not regressed), and separately flagged a pre-existing, not-introduced-by-this-diff hardcoded `530` int literal at `internal/testenv/testenv_test.go:990`. QA independently found the same `testenv_test.go:990` literal and, more significantly, that this story's own maintenance notes for the 1.5.2 bump were initially missing from `docs/TASKS.md` while `README.md`/`docs/PLAN.md`/`docs/api-schema.md` had already been updated; both findings are addressed by this note itself and by changing line 990 to `r.Equal(DefaultAPIVersion, strconv.Itoa(version.Version.API))`, matching the constant-derived pattern already used for the mock body one line above. Product confirmed F-S61 remains the correct home for this note (no new story warranted, consistent with the 1.5.1 precedent) and, using the real InvenTree 1.5.2 release notes (SVG-attachment sanitization, Attachment-API user-permissions validation, a SupplierPart `pack_quantity` pricing-recalculation fix, and an unrelated PO-receive UI fix), confirmed by grepping `internal/upload` and the attachment tool surface that inventree-mcp performs no independent SVG/content-type sanitization and no independent per-model attachment authorization of its own — both server-side hardening patches are pure pass-through tightening with no inventree-mcp contract or behavior change, though a caller whose InvenTree user previously received an incorrectly-permissive response may now see a new (correct) `403`. The pricing fix is a read-through `pricing_min`/`pricing_max` value change on `get_part`, already covered by the existing patch-only-bump editorial note in `docs/api-schema.md`. Infosec found no findings: the diff touches no auth/token/envelope/credential-boundary code, the Docker pin remains an explicit non-digest non-floating tag, and neither the SVG-sanitization nor the Attachment-API permission patch is duplicated or depended upon by inventree-mcp's own upload/attachment code (confirmed by grep: no SVG handling exists anywhere in the module, and the attachment tools are thin pass-throughs with no independent authorization logic). Focused reruns after the `testenv_test.go:990` fix were limited to `internal/testenv`/`internal/buildinfo` (see Validation) since no other file changed; all findings were resolved in the same change before this note was recorded.
- InvenTree 1.5.2 maintenance residual risk: this is a same-API (`530`) patch release (InvenTree `1.5.2`: SVG-attachment sanitization and Attachment-API permissions-validation security patches, a SupplierPart `pack_quantity` pricing-recalculation fix, and an unrelated PO-receive UI fix), so no schema refresh, endpoint-manifest update, or frontend-route re-verification was performed or is believed necessary; the `docs/api-schema.yaml`/`docs/api-schema.md` schema-fetch provenance still reflects the original `1.5.0`-era fetch. A caller whose InvenTree user previously received an incorrectly-permissive Attachment API response under 1.5.1 may see a new `403` after this bump on a request that previously (incorrectly) succeeded; this is expected upstream correctness, not an inventree-mcp regression, but is undocumented for operators beyond this note. `internal/testenv/testenv_test.go:990`'s now-fixed API-version assertion removes one latent drift risk, but this pin change still does not establish the minimum supported InvenTree version, and if a future InvenTree patch release carries an undocumented behavior change despite an unchanged API version, the affected `docs/operator-recipes.md`/`docs/tool-reference.md` prose would need a targeted correction rather than relying on this note's blanket "unchanged" assumption.
...
- Acceptance:
  ...
  - Worker concurrency is bounded and centrally controlled.
  - Cancellation and deadline identity are preserved.
...
- Acceptance:
  - Public tools use resource-specific typed schemas and batch inputs; no generic resource-type escape hatch is exposed.
  - Each item is preflighted and represented in a reviewed batch plan before writes.
  - Per-item no-ops, validation failures, upstream failures, and successful read-backs are reported.
  - A partial batch never claims atomicity and can be safely resumed from failed items.
...
- Residual risk: none beyond the documented coverage gap above; `bulkUpdateMaxItems`/`bulkUpdateConcurrency` are placeholder defaults explicitly deferred to F-S81 for live-evidence tuning. Resolved by F-S81: both became operator-configurable `Config.BulkMaxItems`/`BulkConcurrency` (unchanged 25/4 defaults), and the fixed internal constants were removed.
...
- Acceptance:
  - Batch inputs are typed per resource and cannot request lifecycle transitions through metadata tools.
  - Current-state plans identify every selected order or line and bind all requested field changes.
  - Per-item validation, no-op, failure, ambiguity, and read-back results are returned without claiming cross-record atomicity.
  - Bounded parallel execution preserves request deadlines, cancellation, and existing upstream error classification.
...
- Acceptance:
  - Attachment metadata batches preserve the existing field allowlist, URL safety, upload boundaries, and exact read-back contract.
  - Parameter batches bind template identity, target object/row identity, value shape, overwrite intent, and current state.
  - Duplicate rows, incompatible templates, no-op values, stale state, and missing targets are reported per item.
  - Batch execution is bounded, cancellable, fail-closed, and never claims atomicity.
...
- Acceptance:
  - Representative batches run with bounded concurrency independent of model tool-call pacing.
  - Tests cover fast responses, slow responses, mixed latency, cancellation, deadline expiry, connection reuse, upstream 429/408/425, and ambiguous response loss.
  - Results provide enough progress and per-item evidence to resume safely without dumping unbounded records into MCP context.
  - Concurrency and batch-size limits are explicit, configurable where appropriate, and documented.
  - No automatic retry is performed for ambiguous mutation outcomes.
  - Throughput evidence records client/orchestration time separately from upstream request time.
  - Operator recipes and tool references explain batch approval, partial completion, resume behavior, and the absence of cross-record atomicity.

Tasks:

- [x] Add `BulkMaxItems`/`BulkConcurrency` to `internal/config` (env/YAML/CLI/validation) following the F-S75 layering pattern, and wire them through `tools.Dependencies` and `cmd/inventree-mcp/main.go`.
- [x] Add an `OnProgress` hook and `Timing` (orchestration vs. upstream) result to `internal/batch.Execute`, updating its call sites in `internal/tools` to emit MCP progress notifications via `req.Session.NotifyProgress` (best-effort, throttled) and to surface aggregate timing evidence in each bulk tool's output.
- [x] Extend `internal/inventree/errors.go`'s `classifyStatus` for HTTP 408 and 425 with an explicit, tested classification decision.
- [x] Add deterministic fake-transport coverage for fast/slow/mixed latency and upstream 429/408/425 behavior, extend `internal/batch`'s orchestration-level tests for progress/timing/bounded-concurrency-independent-of-pacing, and add one lightweight connection-reuse assertion.
- [x] Extend the Testcontainers live suite with a batch-shaped ambiguous-response-loss subtest (reusing `loseMutationResponseTransport`) and one representative live throughput observation.
- [x] Update `docs/PLAN.md`, `docs/tool-reference.md`, and `docs/operator-recipes.md` for the new config knobs, progress notifications, timing evidence, and resume/partial-completion/no-atomicity guidance; cross-reference F-S77's resolved residual-risk deferral.

Implementation: `bulkUpdateMaxItems`/`bulkUpdateConcurrency` (previously fixed constants of 25/4 shared by all bulk tools) became `Config.BulkMaxItems`/`BulkConcurrency` (`internal/config/config.go`, `config_file.go`), following F-S75's exact YAML/env/CLI layering, unchanged 25/4 defaults, validated `>0` and capped at 500/64. They flow through `tools.Dependencies` and `cmd/inventree-mcp/main.go`'s two write-enabled `Dependencies` literals. `internal/batch.Execute` gained `ExecuteOptions.OnProgress func(done, total int)` (fires once per terminal item, including not-attempted/cancelled ones) and now returns `(results, Timing{Orchestration, Upstream time.Duration})`, where `Upstream` sums each item's own duration independently so it exceeds `Orchestration` under real concurrency. New `internal/tools/bulk_progress.go` adds `bulkProgressReporter` (delivers `OnProgress` as best-effort `ServerSession.NotifyProgress` calls, throttled to 250ms except the always-sent final `done==total`, silent when the caller attaches no progress token) and `BulkTimingEvidence` (`orchestration_ms`/`upstream_ms`/`item_count`/`concurrency`, the last clamped to `min(configured limit, item_count)` so it reflects workers actually engaged) surfaced as an additive `timing` field on every bulk tool's confirmed output. It also adds `effectiveBulkMaxItems`/`effectiveBulkConcurrency` helpers (fall back to the 25/4 defaults when `Dependencies` leaves the fields unset) used by all 12 call sites, added after this exact gap caused a real bug in the Testcontainers fixture (see Review). `internal/inventree/errors.go`'s `classifyStatus` now explicitly classifies HTTP 408 as `ErrorKindRateLimit` (same back-off semantics as 429) and HTTP 425 as `ErrorKindUnexpected` (documented as deliberate, not a default-case oversight); confirmed no code branches on `ErrorKind` to drive retries, so no automatic-retry behavior exists anywhere before or after this story. `docs/tool-reference.md` and `docs/operator-recipes.md` document the two config knobs, progress notifications, the `timing` field's precise interpretation (including that `bulk_propagate_part_parameters` is unaffected by any of this), and resume/partial-completion/no-atomicity guidance; `docs/PLAN.md`'s `internal/batch` boundary note was extended accordingly.
- Validation: `go build ./...`, `go vet ./...`, `golangci-lint run ./...` (0 issues), `gofmt -l .` (clean), `go mod tidy -diff` (clean), `git diff --check` (clean), `go generate ./internal/tools/...` then no diff on `docs/tool-manifest.json`. `go test ./internal/tools/... ./internal/batch/... ./internal/config/... ./internal/inventree/... ./cmd/... -race` passed (including repeated `-count=3`/`-count=10` runs of the new connection-reuse and progress-throttling tests to check for flakiness). The full Docker-backed `TestMilestoneHappyPathToolsAgainstInvenTree` suite against pinned InvenTree 1.5.1 passed twice end-to-end after a data-race fix found during this story's own live validation (see Review), including the new batch-shaped `bulk_stock_updates/metadata` response-loss subtest (one item's PATCH response dropped and recovered via read-back while a sibling item in the same confirmed call applies normally) with positive `orchestration_ms`/`upstream_ms` evidence.
- Review: full Go/QA/Product/Infosec panel completed per `docs/reviewers.md`'s "adding new mutating workflow tools" trigger (this story touches all 12 existing mutating bulk tools' output contracts and shared execution primitives). Go found a Medium issue (the new `effectiveBulkMaxItems`/`effectiveBulkConcurrency` defensive-default pattern relies on every future call site remembering to use the helper rather than the raw `Dependencies` fields; mitigated with an explicit doc comment on the fields directing readers to the helpers, since `Dependencies` has no constructor anywhere in this codebase to centralize the normalization further) and two Low style issues (both fixed: a test-only constant duplicating the production default now references it directly; `bulkProgressReporter.report`'s lock now covers the `NotifyProgress` call itself so concurrent reports can't reorder on the wire). QA found a Medium test-robustness issue in the new connection-reuse test (a transient extra dial is possible under scheduler contention even with `MaxIdleConnsPerHost` raised, since Go's transport dials immediately rather than waiting for an idle connection; fixed with a small explicit tolerance and comment explaining why) and confirmed as correct-not-off-by-one the `bulk_upstream_status_test.go` GET-call-index reasoning, the live-fixture isolation, the `atomic.Bool` race fix, and the 429/408/425 acceptance coverage; QA also noted as an accepted, documented residual that only `bulk_update_parts` gets dedicated MCP-boundary progress/upstream-status end-to-end coverage since the wiring is byte-for-byte identical across all 12 call sites. Product found a Medium issue (the `timing.concurrency` field reported the configured limit rather than workers actually engaged, which could overstate parallelism for a batch smaller than the configured limit; fixed by clamping it to `min(limit, item_count)` in `bulkTimingEvidence`, with the corresponding test assertion and documentation wording corrected) and two Low documentation issues (both fixed: `classifyStatus`'s 408/425 cases now carry explicit rationale comments; `docs/operator-recipes.md` now states `bulk_propagate_part_parameters` is unaffected by these settings and clarifies the `timing`-interpretation guidance against the precise `item_count > concurrency` condition instead of the vaguer "many-item" phrasing). Infosec found no blocking issues (confirmed progress-notification text and the `timing` field carry only tool names/counts/durations, no record content or upstream error text; confirmed the `atomic.Bool` fix is genuinely race-free; noted as informational, already covered by existing wording, that raising both config caps together multiplies fan-out). All actionable findings were fixed; rebuilt/relinted/retested after each fix.
- Residual risk: none blocking. Accepted, documented residuals: (1) only `bulk_update_parts` has dedicated MCP-boundary progress/upstream-status-code end-to-end tests, since the wiring (`newBulkProgressReporter`/`bulkTimingEvidence`) is identical across all 12 bulk-tool call sites — testing the shared mechanism once was judged sufficient rather than duplicating it 12 times; (2) upstream 429/408/425 are exercised only via deterministic fake transports, never against live pinned InvenTree, since there is no lever to make a real InvenTree instance emit those codes on demand.
...
- Scope: investigate and harden `internal/testenv.waitForWorker`, the Docker-backed InvenTree worker health probe used by Testcontainers-based integration tests, against startup failures observed under concurrent Docker-heavy CI packages (PR #210 failed twice waiting on `invoke worker-health` with `context deadline exceeded`).
- Evidence: `waitForWorker` ran every `invoke worker-health` exec directly against the shared overall `startupCtx` with no per-attempt bound. Under CI Docker-daemon contention a single blocked `Exec` call could consume the whole remaining startup deadline, skipping the intended 250ms retry loop entirely and surfacing only a generic `context deadline exceeded` with no worker state or log context — consistent with the PR #210 failures. This is inferred from code inspection of the affected path, not reproduced under measured concurrent load; no attempt was made to force the exact daemon-contention condition, since the code path alone is sufficient to explain the observed symptom and the fix removes the hazard regardless of its precise trigger.
- Acceptance:
  - Package-level Docker concurrency's contribution to worker startup failures is documented (this section) with the specific code path implicated.
  - Each `invoke worker-health` attempt is bounded by its own timeout (`workerHealthProbeTimeout`, 10s) derived from the caller's context, so one blocked Docker exec cannot consume the remaining overall `StartupTimeout` budget; the outer retry loop and overall deadline are otherwise unchanged.
  - On terminal startup failure, the returned error includes worker container state (status/exit code/OOM) and a bounded tail of recent container logs, gathered with independent per-call timeouts since the caller's context has already expired.
...
- [x] Document whether package-level Docker concurrency contributes to worker startup failures.
- [x] Bound each worker-health exec attempt so one blocked Docker exec cannot consume the whole startup deadline.
- [x] Add worker/container state and bounded log-tail diagnostics to the terminal startup-failure error.
- [x] Add deterministic tests for probe timeout, retry, and startup failure classification using a fake container.
...
- Acceptance:
  ...
  - Tracing is opt-in through documented YAML/environment/CLI configuration and supports OTLP/gRPC and OTLP/HTTP export, including endpoint, TLS, headers/authentication, sampling, batching, and shutdown-flush behavior as appropriate.
...
- Residual risk: none blocking. If a packaged install from an earlier release does exist somewhere despite the operator's confirmation, its operator would need to independently discover that `/etc/inventree-mcp/inventree-mcp.env` is orphaned and unread after upgrading, since detailed migration/secure-deletion guidance was intentionally not added. F-S87 (non-root packaged service identity) was a separate, previously-existing gap, since resolved.
...
- Depends on: F-S86.
- Progress: identified as a gap while implementing F-S86 — the packaged unit had no `User=`/`Group=`/`DynamicUser=` directive and no maintainer script created a system user, so the packaged service ran as root by default. Implemented on branch `claude/f-s87-f5c2c6`. Operator chose a package-created static system user/group over `DynamicUser=yes` (decision recorded below and no longer open): `DynamicUser=yes`'s allocated UID/GID is not guaranteed to persist across reboots unless the unit also declares a `StateDirectory=`/`CacheDirectory=`/`LogsDirectory=`, which this service does not otherwise need, so it risked `config.yml` ownership drifting after a reboot. A static `inventree-mcp` system user/group gives a deterministic identity that survives reboots.
- Scope: run the packaged systemd service as a dedicated non-root identity instead of root. Evaluate `DynamicUser=yes` versus a package-created static system user/group (created in `preinstall.sh` and removed in `postremove.sh` across `deb`, `rpm`, and `apk`), and pick one with a documented rationale. Keep `/etc/inventree-mcp/config.yml` (which may contain the InvenTree token and OAuth key material) readable only by root and the service identity.
  - Implementation note: user/group creation lives in a new `preinstall.sh` (nfpm `preinstall` → deb `preinst`, rpm `%pre`, apk `.pre-install`), not `postinstall.sh` as originally sketched, because `preinstall.sh` runs before package files unpack — required for nfpm's per-file `owner`/`group: inventree-mcp` metadata on `config.yml` to resolve to a real UID/GID at install time. `postinstall.sh` additionally re-applies `inventree-mcp:inventree-mcp` ownership and mode `0600` to `config.yml` on every install/upgrade, covering a pre-F-S87 root-owned conffile a package manager's conffile-preservation logic left untouched.
- Acceptance:
  - The packaged systemd unit runs the service as a non-root identity, with a documented choice and rationale. — done: `packaging/systemd/inventree-mcp.service` sets `User=inventree-mcp`/`Group=inventree-mcp`; rationale recorded above and in `docs/PLAN.md`, `packaging/README.md`, README.
  - `preinstall.sh` creates the static system user (and `postremove.sh` removes it on purge) consistently across `deb`, `rpm`, and `apk`. — done: verified with a `goreleaser release --snapshot --clean` build extracted and inspected on all three formats, plus execution-level tests (`packaging/scripts/scripts_test.go`) that run the scripts with stubbed `dpkg`/`rpm`/`apk`/`getent`/`useradd`/`groupadd`/`adduser`/`addgroup`/`userdel`/`groupdel`/`deluser`/`delgroup` to prove idempotent creation and purge-only-on-final-removal deletion.
  - `/etc/inventree-mcp/config.yml` stays readable only by root and the service identity; ownership/permissions are updated as needed for the chosen approach. — done: nfpm content `file_info` sets `owner`/`group: inventree-mcp`, mode stays `0600`; `postinstall.sh` re-asserts this on every install/upgrade.
  - `packaging/systemd/service_test.go` covers the new `User=`/`Group=`/`DynamicUser=` wiring. — done: `TestPackagedServiceRunsAsNonRootIdentity`.
  - README, `docs/PLAN.md`, and `docs/operator-recipes.md` packaged-deployment guidance describe the non-root service identity. — done, plus `packaging/README.md`.
  - Validation: `go test ./...`, `golangci-lint run ./...`, `git diff --check`, and a `goreleaser release --snapshot --clean` prove the built package contents and permissions. — done, see Validation below.
- Validation: `go test ./...` passed. `go test -race ./...` passed across all packages (full suite, including new `packaging/scripts` and extended `packaging/systemd`/`packaging` coverage). `golangci-lint run ./...` reported 0 issues. `git diff --check` was clean. `goreleaser release --snapshot --clean --skip=docker --skip=publish` succeeded; the built `.deb` and `.apk` were manually extracted (`ar`/`tar`, no `dpkg`/`apk` tooling on the macOS build host) and confirmed `/etc/inventree-mcp/config.yml` is `-rw-------` owned by `inventree-mcp:inventree-mcp`, with the `preinst`/`postinst`/`postrm` (deb) and `.pre-install`/`.post-install`/`.post-deinstall` (apk) maintainer scripts carrying the intended content. The `.rpm` was not independently extracted (no `rpm`/`rpm2cpio` on the build host); residual risk noted below.
- Review: Senior Go Developer, Senior QA / Test Architect, Senior Product Manager, and Senior Infosec Reviewer subagent panel run against the shipped diff (full panel used given the credential-file permission surface). No High/Critical findings. Go confirmed the `set -eu` control flow, nfpm hook-ordering, and dpkg/rpm/apk argument-convention handling are all correct; suggested (addressed) an explicit failure message in `preinstall.sh` when neither `useradd`/`adduser` nor `groupadd`/`addgroup` exists, and a comment documenting that package-manager detection is presence-based (`postremove.sh`). QA found (addressed) that the original script tests only checked syntax and substrings, not actual control flow; added execution-level tests in `packaging/scripts/scripts_test.go` that run the scripts against stubbed system tools to prove idempotency and purge-vs-upgrade dispatch. Product found (addressed) that this story's own status/validation/review/residual-risk/checklist text, its "Open decisions" bullet, and its Scope/Acceptance wording (`postinstall.sh` vs. the actual `preinstall.sh`) were stale relative to the shipped diff — this rewrite resolves that — and suggested (addressed) noting the privileged-listen-port upgrade edge case in `docs/operator-recipes.md`. Infosec found no disclosure window and confirmed the `inventree-mcp` account is properly locked down (no shell, no home, no password); flagged as a residual risk (not fixed, see below) that deleting the system user/group on purge/final removal deviates from the common distro convention of never removing system accounts, to avoid future UID/GID reuse hazards.
- Residual risk: `postremove.sh` deletes the `inventree-mcp` system user/group on final package removal (matching this story's original acceptance criteria), which is a narrower policy than the common distro convention of never removing system accounts, adopted to avoid a future reinstall or unrelated new account being allocated the same numeric UID/GID and inheriting access to anything still owned by it. Practical exposure is low: the only path ever chowned to that UID/GID is the packaged `config.yml` conffile, which purge/final removal also deletes, and the unit intentionally has no `StateDirectory=`/`CacheDirectory=`/`LogsDirectory=` that could leave other state behind. The built `.rpm` package's contents were not independently extracted and inspected on the macOS build host (no `rpm`/`rpm2cpio` available there); the `.deb` and `.apk` extraction confirmed nfpm applied the same shared `owner`/`group`/`scripts` configuration correctly, and rpm scriptlet ordering (`%pre` before file install) is documented to behave the same way, so this is assessed as low incremental risk rather than a blocking gap — a live `rpm -i` install/upgrade/erase cycle in a throwaway Fedora/RHEL container would close it fully if ever needed. `preinstall.sh` fails closed (non-zero exit) rather than silently skipping identity creation if neither `useradd`/`groupadd` nor `adduser`/`addgroup` is available, which is not reachable on any of the three actual target distributions.

Tasks:

- [x] Decide `DynamicUser=yes` versus a package-created static system user/group and record the rationale.
- [x] Wire the chosen non-root identity into the packaged systemd unit and maintainer scripts.
- [x] Verify `/etc/inventree-mcp/config.yml` ownership/permissions under the chosen approach.
...
- Progress: implemented on `claude/f-s90-9cbef6`, PR [#245](https://github.com/davidvanlaatum/inventree-mcp/pull/245). Operator-approved decisions: single generic `render_component_image` tool with a `family` discriminator (not five per-family tools); resistor color-band encoding covers 4-band and 5-band (6-band temperature-coefficient encoding deferred as a documented future extension); the tool requires `inventree.read` even though it makes no InvenTree API calls, reusing the existing OAuth authenticate-and-scope-check path so it is not exposed unauthenticated over HTTP (operator explicitly asked for "no [InvenTree] scope" not to mean "no auth").
  - Follow-up round (after the initial commit, driven by live operator review of rendered output): text rendering switched from `golang.org/x/image/font/basicfont` (a fixed bitmap font) to Go's own "Go Sans" TTF font (`golang.org/x/image/font/gofont` plus `github.com/golang/freetype/truetype`, both already-available dependencies — `golang.org/x/image` was already direct and `freetype` was already an indirect dependency of `github.com/fogleman/gg`; no new `go.mod` entry), so captions can use real Ω/± glyphs with anti-aliasing.
  - The LED template was completely redesigned: the original circle-dome-on-a-separate-base silhouette read as "a mashup of a side and top view" per the operator; it is now one continuous `gg` path (dome arc plus straight-sided cylinder) with a chamfered corner as the polarity indicator and a "+" mark beside the anode lead.
  - The capacitor template was redesigned: markings now print rotated 90 degrees along the can (real electrolytic capacitors print vertically, not horizontally), and the negative stripe is now clipped to the can's own rounded silhouette and shaded like the body instead of reading as a flat panel poking past the curve.
  - The resistor template gained `body_color_hex` (explicit color override), `type` (`carbon_film` default or `metal_film`, each with its own default body color and a luminance-based adaptive outline), `power_rating_watts` (feeds the `show_label` caption and, only when `size` is not also explicit, picks a larger size preset — a real convention, never inferred), and `show_label` (a bold value-and-tolerance line above the body plus a caption below with type/wattage/band-count/band names, every value already in the request or derived from it). The LED template gained `show_label` (size/lens-color/lens-finish caption).
  - Two real rendering bugs were found by the operator looking at output — neither was caught by any test, since the original suite never inspected drawn artwork, only structural invariants (dimensions, background-fill corner pixel): (1) `github.com/fogleman/gg`'s gradient color interpolation produces a black artifact when a gradient's stops differ in alpha — worked around by keeping every gradient in the package fully opaque (fading through color only, never alpha); (2) `fuseGlassColor` and `fuseMarkingBackColor` in `internal/render/fuse.go` were constructed as direct `color.RGBA{R,G,B,A}` literals with straight (non-premultiplied) RGB values and alpha < 255 — invalid for Go's alpha-premultiplied `color.RGBA` type (every component must be `<= alpha`), which rendered as a near-black glass tint and marking backing panel. Fixed with a new `straightRGBA(r,g,b,a)` helper (`internal/render/colors.go`) that correctly premultiplies; documented as the required construction pattern for any future non-opaque color in the package.
  - In response to that gap, added `internal/render/samples.go` (`Samples`, 25 named example configurations spanning all five families and the new fields), `cmd/generate-render-samples` (renders them to the checked-in `docs/images/render-samples/*.png` gallery and writes `docs/render-samples.md`), and `internal/render/samples_test.go`'s `TestRenderSamplesMatchCheckedInGallery`, which re-renders every sample and compares it byte-for-byte against the checked-in PNG — a real visual-regression check, not just "did it error." Verified empirically that Go's test cache cannot mask a stale checked-in image: swapped a gallery PNG for different bytes without touching any `.go` source and confirmed `go test` did not report `(cached)` and correctly failed, then passed again after restoring the file (this relies on `cmd/go/internal/test`'s file-open tracking, keyed by size+mtime rather than content hash, which is why any real edit — not just a content-hash match — reliably invalidates it).
  - The new fields were wired into the MCP tool contract itself (`internal/tools/component_render_tools.go`'s `RenderResistorInput`/`RenderLEDInput`), with a taller default canvas height when `show_label` is set and the caller does not also supply an explicit `height`.
...
- Tasks:
  - [x] Define the renderer contract and the initial family-specific parameter schemas.
  - [x] Implement the five initial deterministic templates and bounded PNG encoding.
  - [x] Add validation, rendering, metadata, MCP contract, and output determinism tests.
  - [x] Align plan, API/capability notes, tool reference, operator recipe, generated metadata, and task documentation.
  - [x] Run applicable validation and the required Go/QA/Product review panel.
- Validation: `go build ./...`, `go vet ./...`, `gofmt -l .` (clean), `golangci-lint run ./...` (0 issues), `go mod tidy` (moved `github.com/fogleman/gg` to the direct-require block after the initial commit, then `github.com/golang/freetype` after the follow-up round's font-engine switch started importing `github.com/golang/freetype/truetype` directly — both were already transitive dependencies before this story, so neither is a new module), `go test ./internal/render/... -cover` (97.4%) and `go test ./internal/tools/...` (all pass, including `TestRenderSamplesMatchCheckedInGallery`'s 25-sample visual-regression suite, the new resistor `type`/`power_rating_watts`/`show_label` tool-contract tests, and the round-2-review-driven `TestSizeForPowerRating`/`TestResistorExplicitSizeOverridesPowerRating`/`TestStraightRGBA`/`TestNoInvalidPremultipliedColorLiterals`), `go test ./... -race` (all packages pass, run three times across the story: after the initial commit, after the visual-redesign follow-up, and after the round-2 review's test additions). `docs/api-schema.md` is unchanged: this story makes zero InvenTree API calls, so there is no endpoint/capability surface to record there.
- Review: two full Senior Go Developer / Senior QA / Test Architect / Senior Product Manager subagent review rounds completed (minimum panel for a Go + test + operator-facing-tool change per `docs/reviewers.md`; the full infosec-inclusive panel was not run because this story adds no OAuth/HTTP-auth behavior, no new mutating tool, and no staff-scoped/administrative read — it is a local-only, read-scoped rendering tool).
  - Round 1 (initial commit): Go found no correctness issues in the orientation/rotation math, the IEC 60062 resistor value-to-band decomposition, or determinism, plus one hygiene finding (`github.com/fogleman/gg` listed indirect instead of direct) fixed via `go mod tidy`. QA flagged six coverage gaps (input-sensitivity, `MaxOutputBytes`, dimension boundaries, rotation direction, mixed-case hex rejection, markings-length boundary), all addressed with new tests. Product flagged the "don't approximate an unsupported family" guardrail existing only in `docs/operator-recipes.md` and not the tool's own schema text, addressed by duplicating it into `family`'s jsonschema description and the tool description; separately noted capacitor's `diameter_mm`/`height_mm` naming drops the `body_` prefix used by the axial families, verified as a non-issue since the MCP SDK's generated schema sets `additionalProperties:false` (a wrong field name is a fail-closed schema rejection, not a silent drop).
  - Round 2 (follow-up: font engine switch, LED/capacitor redesigns, new resistor/LED fields, the two bug fixes, the gallery/regression-test infrastructure): Product re-review confirmed `type`/`power_rating_watts` stay within scope (a narrow two-value manufacturing convention and a genuinely caller-supplied, never-inferred input, respectively) and that the `power_rating_watts` → size-preset precedence (explicit `size` always wins) is implemented consistently in both `internal/render/resistor.go` and `internal/tools/component_render_tools.go`'s `resolveRenderCanvas`; flagged that `power_rating_watts`'s own jsonschema text lacked the "don't infer" guardrail present in `docs/operator-recipes.md` (the same gap class as round 1's `family` finding), addressed by adding it directly to the field's schema string; separately flagged `docs/operator-recipes.md`'s `power_rating_watts` bullet omitting the size-preset side effect, addressed by adding it. Confirmed the checked-in gallery plus byte-for-byte regression test is a proportionate response to the two bugs that shipped past round 1 undetected, and that `show_label` captions make no unsupported claims. Go re-review hand-traced the LED `dc.DrawArc` angle math for both `cathode_side` branches (not just a visual spot-check) and confirmed it is genuinely correct continuous-path geometry, not an accident of the chosen angles; confirmed both bug-class fixes are complete with no recurrence elsewhere in the package (every non-`A:0xff` `color.RGBA` literal goes through `straightRGBA`, every gradient stop stays opaque); confirmed the resistor field-wiring precedence is correct and consistent in both `internal/render/resistor.go` and `resolveRenderCanvas`; confirmed `cmd/generate-render-samples`'s repo-root resolution and gallery ordering are sound; confirmed the new `gg.Clip()`/`dc.RotateAbout()`/font-measurement code paths are deterministic (no randomness, time, or goroutines), consistent with the passing gallery regression test. One non-blocking note: `resolveRenderCanvas` computes its `show_label` canvas sizing before `requireOnlyFamily` validates family/payload exclusivity, so a malformed request does slightly wasted work before being correctly rejected — not a correctness issue, left as-is. QA re-review flagged two real gaps and confirmed everything else solid (both `cathode_side`/`negative_side` mirror cases are covered end-to-end by the gallery test, no flakiness or non-determinism in the new tests): `sizeForPowerRating` and the "explicit `size` overrides `power_rating_watts`" precedence had no direct test (only an indirect SHA-256-differs check), and `straightRGBA`/the premultiplied-alpha invariant that caused the fuse bug had no direct regression test. Both addressed: `internal/render/resistor_test.go` gained `TestSizeForPowerRating` (a threshold table test) and `TestResistorExplicitSizeOverridesPowerRating` (a dimension-level check — an explicit `small` size with `power_rating_watts: 5` renders byte-identical to plain `small`, proving the wattage-derived preset is ignored, not just that the hash differs from something); `internal/render/colors_test.go` (new) gained `TestStraightRGBA` and `TestNoInvalidPremultipliedColorLiterals`. Also added light direct tests for the other previously-untested pure helpers (`internal/render/common_test.go`, new: `TestVerticalLayoutBand`, `TestFitFontSize`, `TestMixColor`) as a low-priority completeness pass QA also noted. Coverage: `go test ./internal/render/... -cover` now 97.4%.
  - Round 3 (CI-caught after pushing round 2): CI's `test` job failed `TestRenderSamplesMatchCheckedInGallery/fuse_3_15a_slow_vertical_transparent` — the strict byte-for-byte comparison did not survive contact with a different CPU architecture than the one the gallery was generated on. Investigated by reproducing under an emulated `linux/amd64` Docker container (the local dev machine and the checked-in gallery are both arm64; GitHub Actions runners are amd64) after registering QEMU binfmt handlers (`tonistiigi/binfmt`), which reproduced the exact CI failure consistently. Pixel-diffed the arm64-checked-in PNG against the amd64-rendered one: exactly 1 pixel out of 54,400 differed, by 2/255 in one color channel — `gg`'s floating-point anti-aliasing rasterizer rounding an edge pixel differently between architectures, not a logic defect (confirmed: all 24 other samples matched exactly; re-running the full 25-sample suite under the same amd64 emulation after the fix below passed cleanly). This means the story's "identical input always produces byte-identical PNG output" claim was overstated: it holds within a single build (same Go toolchain, same GOARCH — `TestRenderDeterministic` still verifies this exactly) but not necessarily across CPU architectures. Fixed by rewriting `TestRenderSamplesMatchCheckedInGallery` to decode both PNGs and compare pixels within a small bounded tolerance (0.1% of pixels, minimum 4, may differ by up to 8/255 per channel) instead of raw byte equality — chosen with a wide margin above the observed 1-pixel/2-magnitude noise while remaining far tighter than the two real bugs this test exists to catch (which differed by large regions and 100+/255 magnitude). Corrected the "byte-identical" claim everywhere it appeared unscoped: `internal/render/render.go`'s package doc comment, `docs/PLAN.md`'s Determinism and Visual regression coverage bullets, `docs/tool-reference.md`'s tool description and gallery-link sentence, and `cmd/generate-render-samples`'s generated `docs/render-samples.md` intro text (regenerated; no PNG bytes changed, only the sentence). Re-verified the full 25-sample suite passes under both native arm64 and emulated amd64 after the fix.
- Residual risk: package-variant `size` presets and derived resistor color bands are illustrative only; a caller that wants true-to-scale imagery must supply the optional paired dimension fields, and even then the renderer does not claim absolute canvas-to-millimeter accuracy. Capacitor's `diameter_mm`/`height_mm` field names intentionally drop the `body_` prefix used by the two axial families (radial vs. axial geometry differ); an agent guessing the axial vocabulary for capacitor gets a fail-closed schema rejection rather than a silently ignored field, so this is a minor discoverability rough edge, not a correctness risk. 6-band (temperature-coefficient) resistor coverage remains a documented future extension, not implemented here. The two rendering bugs found and fixed during the follow-up round (a third-party gradient-library alpha-interpolation defect, and an invalid premultiplied-color construction) were both undetected by the original test suite and only caught by direct visual review; `TestRenderSamplesMatchCheckedInGallery` now guards against a recurrence of that specific failure mode (silently wrong pixel output that still passes every structural check), but is not a substitute for occasionally looking at the actual rendered gallery.
...
- Evidence: CI's `test` job intermittently panicked with a nil-pointer dereference at `internal/inventree/client_methods_integration_test.go:395`, inside the `writes` subtest, at `r.Equal(mpn, *supplierPartDetail.MPN)` — `supplierPartDetail.MPN` was `nil` despite the preceding `UpdateSupplierPart` PATCH and `GetSupplierPartDetail` GET both returning no error. The exercised code path was unchanged since 2026-08-15 (commit `3d423307`); PR #232 only bumped the pinned InvenTree Testcontainers image and touched docs plus one unrelated test literal. The failure did not reproduce locally against the same `inventree/inventree:1.5.2` image, nor on a third CI attempt (attempt 1 failed on an unrelated worker-health timeout, attempt 2 hit this panic, attempt 3 passed clean). This repo's Testcontainers `test` job has a documented history of baseline flakiness under CI resource contention (F-S83; also a prior instance of the same job failing then passing on retry on an unrelated branch, run `32839719047`). Server-side, InvenTree's `SupplierPartSerializer.MPN` field is `serializers.CharField(source='manufacturer_part.MPN', read_only=True, allow_null=True)` — a synchronous, non-cached, direct FK dereference computed at serialization time, not a value InvenTree's background worker recomputes asynchronously — so there is no positive evidence of a genuine eventual-consistency race on the InvenTree API side that would justify a retry/backoff workaround; the more likely explanation is CI resource contention (category (b)/ambiguous from the operator's framing), consistent with the single non-reproducing occurrence. F-S43's residual-risk note already documents that derived `MPN` "can change independently of the sourcing-link record; callers must treat [it] as read-time context rather than stable identity" (`docs/TASKS.md`, F-S43 section) — that note is about API-level product behavior for MCP tool callers (the value can legitimately change between reads), and does not itself indicate a write-then-immediate-read race; it does not conflict with the conclusion here, but is called out for anyone revisiting this evidence later.
- Follow-up evidence (caching layers, checked after the operator asked specifically about Redis staleness): InvenTree's `SupplierPartDetail` view (`RetrieveUpdateDestroyAPI` in `company/api.py`) carries no caching decorators (no `cache_page`, `vary_on_headers`, or `etag`); its `prefetch_related('part', 'part__pricing_data')` is a per-request query optimization, not cross-request caching. `company/models.py` and `company/serializers.py` — where `SupplierPart`, `ManufacturerPart`, and the `MPN` field are defined — contain no Django cache-framework usage at all (no `cache.get`/`cache.set`, `@cached_property`, or Redis imports). InvenTree does use Redis (via `django-redis`, wired through `InvenTree.cache.get_cache_config`) as a general-purpose, opt-in cache for specific call sites elsewhere in the codebase — notably `PartPricing`, which genuinely is async-recalculated and is prefetched right alongside this same queryset (`part__pricing_data`) — but `MPN`/`manufacturer_part` are not part of that cached/async-recalculated surface. This rules out both an HTTP-level response cache and a Redis-backed model/query cache as explanations for this specific field, on top of the already-ruled-out async-worker-recompute hypothesis; it does not rule out connection-pool or transaction-timing effects under CI load, which remain the most plausible remaining explanation alongside plain CI resource contention.
- Decision: harden the test to fail with a clear, isolated assertion message instead of an opaque whole-binary panic, and capture the previously-discarded PATCH response for future bisection; do not add a speculative retry/backoff, since the root-cause evidence points at CI contention or an unproven, unreproduced hypothesis rather than a confirmed product-side race that a retry would correctly paper over.
- Acceptance:
  - `internal/inventree/client_methods_integration_test.go`'s `writes` subtest captures the `UpdateSupplierPart` PATCH response instead of discarding it, and asserts `r.NotNil(...)` with a diagnostic message (supplier-part PK, expected MPN, manufacturer-part PK) before dereferencing `MPN` on both the PATCH response and the subsequent `GetSupplierPartDetail` response, so a recurrence reports which of the two responses lost the value and produces a readable `require` failure instead of a test-binary panic.
  - No retry/backoff or other behavioral change is introduced; the change is diagnostic-only.
- Validation: `go build ./...`, `go vet ./...`, and `gofmt -l` clean. `go test ./internal/inventree/... -run TestClientMethodsAgainstInvenTree -timeout 20m` passed locally against `inventree/inventree:1.5.2` (135s). Full `go test ./... -race` passed across all packages.
- Review: Senior Go Developer and Senior QA / Test Architect subagent review completed (minimum panel for a Go + test change per `docs/reviewers.md`; the full four-role panel was not run because this is a diagnostic assertion change to an existing test, not a change to Testcontainers integration architecture per `docs/reviewers.md`'s "Review Timing" guidance). Go: no findings — the `r.NotNil`-before-dereference pattern matches existing precedent elsewhere in the same file, no shadowing/compile issues, no behavior change beyond diagnostics. QA: no findings on the test-logic scope decision (limiting the hardening to the MPN dereference is correct; other nearby bare dereferences in the same subtest echo request-body fields directly rather than reading a cross-object FK-derived value, so they do not share MPN's risk profile) or on message/style consistency; flagged (addressed in this section) that the investigation and rationale were not yet tracked in `docs/TASKS.md`, unlike the F-S83 precedent, and flagged (addressed above) that F-S43's existing MPN residual-risk note should be explicitly reconciled rather than left unaddressed.
- Residual risk: this change does not resolve the underlying CI flake, if a genuine one exists — it only makes a recurrence diagnosable instead of a whole-binary panic. If the hardened assertion fails again in CI with the same evidence pattern (PATCH response already missing MPN vs. only the later GET missing it), re-open this investigation using that new evidence; a repeated PATCH-response-side failure would point away from CI contention and toward a real write-path issue worth escalating upstream.

Tasks:

- [x] Reproduce/characterize the CI-only nil-panic and rule out a local reproduction.
- [x] Confirm server-side `MPN` field semantics (synchronous FK dereference, not async-recomputed) to weigh race vs. CI-contention hypotheses.
- [x] Harden the test assertion to fail cleanly with diagnostic context instead of panicking, without adding unjustified retry logic.
- [x] Validate locally (build/vet/gofmt, targeted integration test, full `go test ./... -race`).
- [x] Run subagent review and address findings.
...
- Progress: implemented on `claude/f-s91-cross-object-tags` from `origin/main` at `fe81dbd`.
...
- Acceptance:
  - `search_tags` accepts an optional `model_type` and a name `search` term, is backed by `/api/tag/` with an explicit `limit`, and returns stable `id`/`name`/`slug` rows; it requires only `inventree.read`.
  - Each covered object's exact-read tool requests the underlying `?tags=true` query flag (or object-appropriate equivalent) so `tags` is present in its exact-read output; list/search tools for the same objects remain concise and do not add `tags`.
  - Each covered object's existing `update_*` tool accepts an ordinary optional `tags` field (whole-array replace; explicit `[]` clears all tags), using the same `inventree.read`+`inventree.write` scopes and non-destructive annotations that tool already publishes — no new `inventree.destructive` requirement, matching the live-pinned finding that InvenTree does not gate object-level tag writes specially.
  - Tag-name matching is case-insensitive InvenTree-side (confirmed by F-S56); tools must not attempt their own client-side case normalization or reject a case-variant of an existing tag.
  - Direct `/api/tag/` entity mutation (rename/delete a `Tag` row) is not exposed by any tool in this story.
  - Pinned integration tests cover `search_tags` (unscoped and `model_type`-scoped), tag assignment/read-back/clear for at least two of the seven covered object types, and the shared-taxonomy behavior (same tag name across two object types resolves to one row); plan, schema, tool-reference, and operator-recipe docs are updated to describe the new field/tool and the deferred object types.
- Validation: `go build ./...`, `go vet ./...`, `golangci-lint run ./...` (0 issues), `gofmt -l` clean, and `git diff --check` all pass. `go test -tags no_integration_tests ./...` passes across every package. `GOFLAGS=-trimpath go test -race -p=1 ./...` passes across every package, including the new `cross_object_tag_search_and_assignment` live subtest (tag assignment/read-back/clear via typed `UpdatePart`/`UpdateCompany`/`UpdateStockLocation`/`UpdatePurchaseOrderDetail` for Part/Company/StockLocation/PurchaseOrder, `search_tags` unscoped and `model_type`-scoped shared-row resolution, a negative-scope check that a tag never assigned to an object type does not leak into that type's scoped search, and explicit `tags:[]` clearing) and live plain-GET/PATCH `?tags=true` omission/inclusion pairs for every one of the seven covered object types (Part/Company/StockLocation already pinned by F-S56; SupplierPart/ManufacturerPart/PurchaseOrder newly added by this story so the omission claim is test-backed, not just asserted). Per-package coverage vs `origin/main`: `internal/tools` 85.0% vs 84.9%; `internal/inventree` 83.6% vs 82.8% (both improved, after closing gaps the review panel found in `search_tags` and the six `tags` PATCH-field call sites).
- Review: full four-role subagent panel (Senior Go Developer, Senior QA/Test Architect, Senior Product Manager, Senior Infosec Reviewer) completed, per this repo's tool-surface trigger for adding new tool surface across seven object types. Go found (High) that the tools-layer postflight verification helpers (`patchMatches` for company/supplier/manufacturer-part, `locationFieldsMatch` for stock location) compared submitted-vs-read-back `tags` with order-sensitive equality, risking a spurious `partial_failure` on any real multi-tag write InvenTree happens to reorder on read-back; fixed with a new order-insensitive `tagSetsMatch` helper plus unit tests proving both the reordered-success and genuine-mismatch-still-fails cases for `update_company`, `update_supplier_part`, and `update_stock_location`. Go, QA, and Product all separately flagged the tools-layer `search_tags` handler and the six `tags` PATCH-field call sites had no unit coverage (only live integration); closed with `tag_tools_test.go` and targeted PATCH-field/omit-vs-clear tests on `update_part` and `update_purchase_order`. QA additionally found `docs/PLAN.md`'s claim that plain-GET/PATCH tag omission was "re-confirmed" for SupplierPart/ManufacturerPart/PurchaseOrder wasn't actually backed by a test (those probes always requested `?tags=true`) and that `GetPurchaseOrderDetail`'s `?tags=true` flag had no unit coverage unlike its six siblings; both closed by adding the missing live plain-request assertions and the missing unit-test case. Product found `docs/tool-reference.md`'s `bulk_update_parts`/`bulk_update_purchase_orders` rows now falsely claimed "full field set" coverage since `tags` is deliberately out of bulk scope, and that `search_tags`' `model_type` field was under-documented relative to the `search_object_parameters` pattern it claims to mirror; both fixed in docs, with `model_type` staying deliberately unvalidated (InvenTree's own shared taxonomy is broader than the seven MCP-covered object types) but now enumerating them as jsonschema/doc examples. Infosec found no blocking issues; confirmed `search_tags` auto-lands in the read-only/`inventree.read` bucket and none of the six touched `update_*` tools had scopes or annotations widened, and suggested (accepted) documenting that `search_tags` has no verified per-object-type read-permission scoping, matching the same unverified caveat `global_search`/`search_owners` already carry. All actionable findings from all four reviewers were addressed in a follow-up commit; Go's one low-severity stylistic note (`UpdatePurchaseOrderDetail` duplicates `Client.Patch`'s request-building logic to add `?tags=true`) was accepted as-is — a single-call-site abstraction was judged not worth the indirection.
- Residual risk: whole-array replace on `tags` is last-write-wins if two callers update the same object's tags concurrently, matching the existing `Attachment` `tags` field's behavior; unreferenced `Tag` rows are never auto-deleted by InvenTree, so `search_tags` results can include names no covered object currently references — callers should treat a `search_tags` match as "a known label," not proof of current usage, without a bounded live cross-check this story does not add. `search_tags` returns rows from the shared `/api/tag/` taxonomy with only ordinary read-access gating, not per-object-type authorization scoping (for example a token lacking view access to a specific object family); this was not exercised live and remains unverified, matching the same unverified caveat `global_search` and `search_owners` already carry for their own endpoints. Tags are non-sensitive business labels (name/slug only), so this is judged low-impact.

### F-S92: Render-Component-Image Family Enum And Anti-Invention Wording

- Status: `Done`
- Issue: [#249](https://github.com/davidvanlaatum/inventree-mcp/issues/249)
- Depends on: F-S90
- Decisions: the operator confirmed the reported `axial_resistor` doc/schema mismatch no longer exists in current code or docs (both already say `resistor`); the remaining actionable ask is exposing `family` as a real JSON Schema `enum` so a caller can discover the five accepted values before a failed call, rather than only from descriptive text. F-S30 previously chose descriptive text over a JSON Schema `enum` for `model_type`, but that was specifically because `model_type`'s vocabulary comes from separate upstream InvenTree API endpoints that could add values independently of this code; `family` has no such upstream dependency — it is a fixed set of exactly five values this server itself defines in `render.Family` — so that rationale does not apply here and an enum is safe. While in this area, the operator also asked to (1) strengthen the schema wording on every other optional color/dimension/marking/rating field across all five families so a calling agent only supplies a value it was actually told, rather than inventing or estimating one, matching the pattern `power_rating_watts` already used, and (2) after confirming `orientation`/`background` already had a backing `internal/render` type the same way `family` did, extend the enum treatment to every other closed-vocabulary field on this tool — `tolerance_label`, the four different `size` vocabularies (resistor/diode/capacitor share small/medium/large; LED and fuse each have their own), resistor `type`, the shared cathode/negative `side`, LED `lens_color`, and fuse `speed` — even though most of those had no backing Go type yet and needed one added to `internal/render`.
- Scope: change `RenderComponentImageInput.Family`, `Orientation`, and `Background`, and every family-specific input struct's closed-vocabulary field, from plain `string` to a distinct `internal/render` type, adding `render.ToleranceLabel`, `render.BodySize`, `render.ResistorType`, `render.Side`, `render.LEDLensColor`, `render.LEDSize`, `render.FuseSize`, and `render.FuseSpeed` (and `Orientations()`/`Backgrounds()`/`BodySizes()`/`Sides()` accessor functions alongside the existing `ToleranceLabels()`/`ResistorTypes()`/`LEDLensColors()`/`LEDSizes()`/`FuseSizes()`/`FuseSpeeds()`) where a type or accessor did not already exist; register a real JSON Schema `enum` for all eleven types via `jsonschema.ForOptions.TypeSchemas`, sourced from these accessor functions so the schema and `internal/render`'s own runtime validation cannot drift apart, replacing `render_component_image`'s automatic schema inference with an explicit `jsonschema.For` call so the overrides apply. `internal/render`'s own `ResistorParams`/`DiodeParams`/`LEDParams`/`CapacitorParams`/`FuseParams` fields and validation logic remain plain `string`/unchanged behavior; only their few inline error messages were reworded to cite the new accessor functions instead of a hardcoded literal list, matching the pattern `ToleranceLabels()`/`ResistorTypes()`/`LEDLensColors()` already used. Also strengthen the jsonschema description wording on every optional `*_color_hex`, `body_length_mm`/`body_diameter_mm`/`diameter_mm`/`height_mm`, `markings`/`markings_text`, and `capacitance_microfarads`/`voltage_rating_v`/`rating_voltage` field across `RenderResistorInput`, `RenderDiodeInput`, `RenderCapacitorInput`, and `RenderFuseInput` to say the value should only be supplied when the caller actually stated it. Does not add tool-surface, change runtime validation behavior, or touch any other tool's schema.
- Acceptance:
  - The registered `render_component_image` tool's input schema has a real JSON Schema `enum` on `family`, `orientation`, `background`, `resistor.tolerance_label`, `resistor.size`, `resistor.type`, `diode.cathode_side`, `diode.size`, `led.lens_color`, `led.cathode_side`, `led.size`, `capacitor.negative_side`, `capacitor.size`, `fuse.speed`, and `fuse.size`, generated from `internal/render` constants/accessor functions so the schema and runtime accepted values cannot drift.
  - The handler's own unknown-`family` fallback error and runtime `switch` continue to work unchanged for direct Go callers (unit tests) that bypass schema validation.
  - Every optional color-hex, physical-dimension, marking/text, and rating field on the five family-specific input structs has jsonschema wording that tells the caller to supply it only when the caller stated an actual value, and to omit it (letting the tool's default or derived value apply) otherwise.
  - `docs/tool-reference.md` and `docs/operator-recipes.md` describe the enum-backed fields and carry the same anti-invention guidance for the affected fields.
  - `docs/tool-manifest.json` is regenerated and shows no unexpected diff (it does not carry per-field schema detail).
  - No package-level test coverage reduction versus `origin/main` for any touched package; new `internal/render` accessor functions have direct unit coverage.
- Tasks:
  - [x] Change `Family`, `Orientation`, and `Background` to their `render` types and add the `jsonschema.ForOptions.TypeSchemas`-based enum schema for `render_component_image`.
  - [x] Add the missing `render` types/accessor functions and extend the enum schema to `tolerance_label`, `size` (all four vocabularies), resistor `type`, `cathode_side`/`negative_side`, `lens_color`, and `speed`.
  - [x] Strengthen anti-invention wording on the affected optional fields across all five families.
  - [x] Align `docs/tool-reference.md` and `docs/operator-recipes.md`.
  - [x] Add direct unit tests for the new `internal/render` accessor functions to close the coverage gap they introduced.
  - [x] Run validation and the applicable reviewer panel; address findings.
- Validation: `go build ./...`, `go vet ./...`, `golangci-lint run ./...` (0 issues), `gofmt -l` clean, `go mod tidy -diff`, and `git diff --check` all pass. `go generate ./internal/tools` produced no unexpected `docs/tool-manifest.json` diff. `go test -tags no_integration_tests ./...` passes across every package, including `docs`' schema/documentation contract tests and the new `TestComponentRenderInputSchemaEnums` (`internal/tools/component_render_tools_test.go`), which asserts the generated schema's `enum` on every listed field equals the corresponding `render` accessor function's output, rather than relying on the ad hoc manual check used during initial implementation. Per-package coverage vs `origin/main` (compared via a temporary detached worktree at `origin/main`, `no_integration_tests` tag): `internal/render` 97.9% vs 97.4% (improved — direct unit tests were added for every new/previously-uncovered accessor function, including `ResistorTypes()` and `Families()`, which QA's review found had no direct coverage), `internal/tools` 85.0% vs 85.0% (unchanged), `docs` 57.1% vs 57.1% (unchanged). `GOFLAGS=-trimpath go test -race -p=1 ./...` passed on the diff prior to the review-driven fixes, including all default-on Testcontainers integration suites; the no-integration suite was re-run clean after those fixes (see above).
- Review: Senior Go Developer, Senior QA / Test Architect, and Senior Product Manager subagent review completed (minimum panel per `docs/reviewers.md` for a Go/schema change with operator-facing documentation impact; full Infosec panel was not run since this story adds no tool surface, auth, upload, or Testcontainers-relevant behavior). QA found (High) `render.ResistorTypes()` had 0% direct test coverage in `internal/render` — no test exercised an invalid resistor `type`, unlike every sibling enum's error path; fixed by adding that case to `TestRenderResistorValidation`. QA also found (Medium) no automated test asserted the generated JSON Schema actually carried the expected `enum` arrays, only a manual ad hoc check; fixed by adding `TestComponentRenderInputSchemaEnums`. QA found (Low) the local `componentRenderFamilyValues` list in `component_render_tools.go` was the one enum source not backed by a `render` package accessor, unlike every other enum in this story; fixed by adding `render.Families()`/`familyOrder` and using it directly. Product independently found the same issue from the other direction (High/Low): `docs/tool-reference.md`'s diode/capacitor/fuse bullets were left without the anti-invention wording that `operator-recipes.md` and the actual jsonschema descriptions already carried, and the handler's unknown-`family` fallback error hardcoded the five constants directly despite a doc comment claiming it stayed "in lockstep" with the schema; both fixed — `tool-reference.md`'s three bullets now carry matching wording, and the fallback error now calls `fmt.Errorf("family must be one of %v", render.Families())` so there is exactly one place the family list is defined. Go reviewer independently verified the `jsonschema.For`/`TypeSchemas` mechanism against the `jsonschema-go` library source (confirming per-field descriptions survive the type-level enum override), confirmed every family-specific `string()` conversion at the `render.XxxParams` call sites is complete and correct, and found no further actionable issues beyond what QA/Product already surfaced. All actionable findings from all three reviewers were addressed in this story; none were rejected.
- Residual risk: none identified; this is a schema-discoverability and wording change with no runtime validation or tool-surface change. `render.ResistorType`, `render.Side`, and the other new types are purely additive to `internal/render`'s public surface and do not change any `render.XxxParams` struct's field types or validation behavior.

### F-S93: Stateless Per-User Bearer Bootstrap For HTTP Clients

- Status: `Done`
- Issue: [#253](https://github.com/davidvanlaatum/inventree-mcp/issues/253)
- Depends on: F-S07, F-S08, F-S09.
- Progress: created from the LAN remote-HTTP MCP client configuration gap. The client configuration accepts a statically configured bearer token, but does not appear to initiate the MCP OAuth flow. This story covers an alternate authenticated mode for multiple LAN users; it does not remove or weaken the existing OAuth path. Implementation on branch `claude/issue-253-83f30f`, rebased onto `main` at commit 937b840 (post-#255).
- Decisions: do not store a per-user token mapping on the MCP server. The server returns an opaque, encrypted and authenticated MCP bearer envelope containing a dedicated InvenTree API credential. The normal MCP endpoint accepts only that MCP-owned bearer envelope, never a raw inbound InvenTree `Token` or `Bearer` credential. The bootstrap operation must be a separate endpoint, use the configured canonical HTTPS origin, avoid logging credentials, and apply bounded request/rate-limit protections. Support both InvenTree authentication forms: HTTP Basic username/password and an existing InvenTree API token using `Authorization: Token ...`; these are separate bootstrap request forms, not an API token placed in the Basic password field. OAuth and stateless bearer bootstrap may coexist in one deployment, with all MCP-owned operational routes under the configured public MCP path prefix so they can share a reverse-proxy route without colliding with InvenTree paths. The configured prefix defaults to `/mcp`, so the default operational routes are `/mcp`, `/mcp/auth/bootstrap`, and `/mcp/oauth/*`; deployments may configure another prefix. OAuth `/.well-known/...` metadata remains at its existing standards-compatible locations. Earlier `/mcp` examples refer to the default or configured prefix, not necessarily a hard-coded path. If dedicated InvenTree token creation fails, bootstrap fails closed and issues no MCP token; the supplied credential is never sealed as a fallback, independently re-checked with `ValidateForEnvelope` before sealing rather than relying only on broker convention. Dedicated InvenTree token names are instance-wide random identifiers such as `inventree-mcp-static-<random>`, with no username; multiple tokens for one user are allowed, and creation retries a bounded number of times on a transient upstream failure (InvenTree's token-issuance endpoint does not actually error on a name-collision; it silently deletes and recreates a same-named token, so the no-rotation guarantee comes from the 128-bit random name, not the retry). MCP bearer envelopes expire after 30 days by default, with the lifetime configurable by the operator. Operator-confirmed during implementation planning: bootstrap-issued envelopes carry the full `supportedOAuthScopes()` set (no consent screen exists for this flow; the user's own InvenTree permissions remain the real authorization boundary), and `BootstrapEnvelopeLifetime` validation requires only `> 0` with no upper ceiling, matching the existing OAuth lifetime fields' validation style.
- Scope: add a production-safe non-OAuth bootstrap flow for a user to exchange an InvenTree username/password or existing API token for a stateless MCP bearer token. The returned token is entered into the remote MCP client's static bearer-token configuration. Each user therefore retains their own InvenTree permissions and credential identity without a shared token or server-side mapping database.
- Acceptance:
  - Add an explicitly configured HTTP authentication mode for stateless bearer bootstrap that can coexist with OAuth, with clear interaction rules and a fail-closed production configuration.
  - Add a dedicated bootstrap endpoint that requires the configured canonical HTTPS deployment, accepts bounded Basic username/password or an InvenTree `Token` credential, validates the supplied credential against the configured InvenTree instance, creates a dedicated InvenTree API token using the existing broker/client capability, and returns only an opaque MCP token; if creation fails, return an error without issuing a token or sealing the supplied credential.
  - Seal the upstream credential in the existing encrypted/authenticated envelope design with explicit issuer/audience, expiry, key ID, and rotation/decryption-state handling. No plaintext JWT or server-side user/token mapping is permitted.
  - Authenticate ordinary MCP requests with the MCP envelope, decrypt it, and use the embedded dedicated InvenTree token for the requested API call. Reject raw InvenTree authorization headers and malformed, expired, wrong-audience, or wrong-key-state envelopes.
...
- Open decisions: none. Request-form dispatch (by the bootstrap request's own `Authorization` header scheme; there is no separate credential-form body, so there is no dual-credential precedence to resolve) and the dedicated-token name prefix (`inventree-mcp-static-<random>`) were settled during implementation and are recorded in Decisions above.
- Tasks:
  ...
  - [x] Run the applicable full validation and auth/security reviewer panel; record residual risk before marking the story Done.
- Validation: `go build ./...`; `go vet ./...`; `gofmt -l .` (clean); `golangci-lint run` on oauth/config/server/inventree/testenv (0 issues); `go test -race ./...` (full suite, all packages pass, including the default-on Testcontainers live bootstrap integration test for both Basic and Token credential forms, each now asserting the sealed dedicated token differs from the supplied credential); `git diff --check`.
- Review: full Go/QA/Product/Infosec panel run per `docs/reviewers.md`. Findings that converged across two or more independent reviewers, or mapped to an already-checked acceptance-criteria bullet, were treated as blocking and fixed: (1) widening `inventree.Credential.Validate()` to accept Basic for the bootstrap flow had also silently widened the OAuth setup page's `credential_scheme` form field beyond its own Token/Bearer `<select>` — closed with an explicit `ValidateForEnvelope` guard in `completeAuthorization`, with a regression test; (2) the bootstrap handler sealed the broker's returned "dedicated" credential without independently validating it — added a `ValidateForEnvelope` check before `Seal`, with a test using a broker that returns the caller's own supplied credential to prove the guard is structural, not just a broker convention; (3) `CredentialFromTokenInfo` still called `Validate()` instead of `ValidateForEnvelope()` — fixed for consistency; (4) zero logging and zero non-disclosure tests on failure paths (an already-checked acceptance bullet) — added minimal outcome-only logging (subject + result, never the credential) via the request-scoped logger, plus failure-path tests for both Token and Basic forms asserting no captured log record or response body contains the submitted secret; (5) the fail-closed test only checked the response body, which cannot detect a credential sealed as ciphertext — added retry-exhaustion call-count assertions and envelope-opening assertions; (6) Testcontainers subtests reused the parent test context, contrary to `AGENTS.md`'s per-subtest-context rule — fixed, and both subtests now open the returned envelope to prove the sealed credential is a genuinely new dedicated token, not the supplied one; (7) the Authorization scheme match was case-sensitive (RFC 7235 treats it as case-insensitive) — fixed with a test; (8) the retry loop's own comment and the docs described a name-collision rationale that doesn't match InvenTree's actual delete-and-recreate semantics — corrected in code comments, `docs/PLAN.md`, and `docs/operator-recipes.md`, and the loop now bails immediately on a cancelled context; (9) the bootstrap endpoint had only IP-based rate limiting, which a distributed attacker can defeat by spreading credential guesses across source addresses — added a second rate-limit dimension keyed by a hash of the submitted credential, with a cross-IP test; (10) the "full scope set" and "response is not cacheable" decisions were undocumented in tests — added a unit assertion, a mux-level write-scoped tool call proving scope enforcement actually grants write, and a `Cache-Control: no-store` assertion; (11) several documentation gaps (missing OAuth-still-required prerequisite, no documented `bootstrap_enabled:false` kill switch, missing F-S10 milestone caveat, unworkable token-cleanup guidance given renewal has no refresh token, no Basic-credential `curl` example, undocumented password-rotation/shared-envelope/Basic-password-handling risks, key-rotation grace window not accounting for `bootstrap_envelope_lifetime`) were fixed in `README.md`, `docs/operator-recipes.md`, and `docs/PLAN.md`. Full reviewer findings are recorded in the implementing session; see Residual risk below for what was explicitly deferred rather than fixed.
- Residual risk: stateless envelopes intentionally trade individual immediate revocation for no server-side mapping and compatibility with clients that only support a static bearer token; the `bootstrap_enabled: false` kill switch and envelope-key rotation are the available emergency mechanisms, both now documented. Production HTTP mode still requires full OAuth client/lifetime configuration even for a bootstrap-only deployment — making OAuth registration independently optional is a larger architecture change than this story's acceptance criteria called for ("coexist with OAuth", not "OAuth-optional"), so it is documented as a current limitation in `README.md` and `docs/operator-recipes.md` rather than implemented. Several reviewer nice-to-haves were deferred as non-blocking: a full env-var/YAML precedence test matrix for the two new config fields (only CLI-flag-level precedence is tested); a startup-time test for `BootstrapServer.Register`'s incomplete-config guard; tests for the `codec.Seal` and `IDGenerator.NewID` error branches; wiring `BootstrapServer.Clock`/`IDGenerator` at the production `server.go` call site for deterministic mux-level expiry/naming tests; JSON-shaped (vs. plain-text) bootstrap error bodies and a `WWW-Authenticate` challenge header; and `BootstrapEnabled` being silently inert outside production HTTP transport, which matches the existing behavior of the OAuth config fields it sits beside. Because bootstrap has no refresh token, a client that renews on schedule permanently accrues one new `inventree-mcp-static-*` InvenTree token per renewal with no way to distinguish live from stale ones from InvenTree's token list alone; this is now documented as the expected steady state in `docs/operator-recipes.md` rather than an edge case, with periodic manual audit as the only mitigation.

### F-S94: Prefix OAuth Endpoints Under The Configured MCP Path

- Status: `Ready`
- Issue: [#260](https://github.com/davidvanlaatum/inventree-mcp/issues/260)
- Depends on: F-S93.
- Progress: identified during live packaged deployment when the reverse proxy exposed OAuth endpoints at the issuer root (`/authorize` and `/token`), where they could conflict with current or future InvenTree routes. The existing F-S93 routing decision already places MCP-owned operational routes under the configured prefix. This is planned as an atomic breaking cutover: deploy the new MCP/reverse-proxy route set together, then have existing clients rediscover metadata or be reconfigured; no dual-route compatibility period is required.
- Decisions: OAuth authorization and token endpoints will be exposed at `<configured-prefix>/oauth/authorize` and `<configured-prefix>/oauth/token`; with the default prefix this is `/mcp/oauth/authorize` and `/mcp/oauth/token`. The configured OAuth issuer URL remains the canonical origin used for metadata and token validation, while its advertised endpoint URLs include the configured prefix. Protected-resource and authorization-server well-known metadata locations remain unchanged and follow the existing path-specific forms: `/.well-known/oauth-protected-resource<resource-path>` and `/.well-known/oauth-authorization-server<issuer-path>` (so the default `/mcp` resource uses `/.well-known/oauth-protected-resource/mcp`, while an origin-only issuer uses `/.well-known/oauth-authorization-server`). The reverse proxy must route the configured prefix and its subpaths to MCP, while ordinary InvenTree paths remain separate. Prefixes are normalized to one leading slash with no trailing slash; empty, root, and prefixes overlapping either well-known metadata or metrics paths are rejected at startup. The canonical MCP root accepts both `/prefix` and `/prefix/` (including their proxy forms), while OAuth, bootstrap, and other prefixed endpoint paths are exact and any trailing-slash variant returns 404; no unsafe POST redirect is used. This is an intentional breaking endpoint migration: the new server exposes no compatibility alias at root-level `/authorize` or `/token`, and clients must rediscover metadata or be reconfigured. No temporary old-route proxying or backward-compatibility support is required; those paths must return 404 and must not be routed to InvenTree.
- Scope: update OAuth route registration, canonical endpoint construction, metadata, path validation, tests, operator documentation, packaged examples, and the Caddy reverse-proxy recipe to use prefixed OAuth endpoints.
- Acceptance:
  - Authorization-code and token handlers are registered only at the configured-prefix OAuth paths, including non-default prefixes.
  - Authorization-server metadata advertises the prefixed authorization and token endpoints, and all authorization/token tests use those URLs.
  - Route validation rejects collisions between the MCP prefix/OAuth subpaths and protected-resource or metrics paths without rejecting ordinary InvenTree paths outside the prefix.
  - A route matrix covers default and non-default prefixes, configured-prefix normalization, accepted `/prefix` and `/prefix/` MCP-root forms, rejected trailing-slash variants for `/prefix/oauth/authorize`, `/prefix/oauth/token`, bootstrap, and other prefixed endpoints, nearby paths such as `/mcpfoo`, empty/root prefixes, prefixes overlapping metrics or well-known metadata, the exact path-specific metadata locations derived from resource and issuer paths (including `/mcp` resource and origin-only issuer forms), arbitrary `/.well-known/*` negative cases, exact `/prefix` and `/prefix/*` proxy behavior, and confirms at both the MCP listener and Caddy/reverse-proxy boundary that root-level `/authorize` and `/token` return HTTP 404, do not reach the new handlers or InvenTree, and have no compatibility alias.
  - The documented Caddy route proxies `/mcp` and `/mcp/*` (or the configured equivalent) plus the unchanged standards metadata paths; it no longer needs root-level `/authorize` or `/token` routes.
  - Existing stateless bootstrap and OAuth connector flows continue to work through the reverse proxy, with no raw InvenTree credential or OAuth token disclosure.
  - Update `docs/PLAN.md`, `docs/operator-recipes.md`, README configuration guidance, packaged examples, and relevant tests/reference docs in the same change.
- Out of scope: changing the OAuth protocol, issuer origin, protected-resource metadata locations, token envelope format, client registration policy, or stateless bootstrap credential/lifetime behavior.
- Tasks:
  - [ ] Update route registration and canonical OAuth endpoint derivation for configured prefixes.
  - [ ] Add unit and integration coverage for default and non-default prefixes, metadata, and collision validation.
  - [ ] Update Caddy/operator/package documentation and examples.
  - [ ] Run full validation and the applicable Go, QA, Product, and Infosec review panel.
- Residual risk: clients or reverse proxies using the old root-level OAuth endpoint URLs will need to rediscover metadata or be reconfigured after deployment; deployments must ensure those old paths are not routed to InvenTree.
...
- Decisions: tool invocation and completion records will be INFO-level structured logs, independently of whether OTEL tracing or metrics are enabled. Establish the request-scoped logger with `dvgoutils/logging.WithLogger` after the active MCP span exists at the HTTP/tool boundary, embedding a server-generated opaque `request_id` (fixed 32-character lowercase hex), transport, tool, source-IP, and valid-span trace fields once; if ID generation fails, fail the invocation closed before dispatch with a safe internal error and emit only a separate startup/error record without pretending to have a request ID. Downstream handlers and the InvenTree client use `logging.FromContext` and derive child loggers with `logger.With(...)` rather than receiving logger parameters through every function. The call sequence is atomically assigned starting at 1, capped at 64, and resets per invocation; when the cap is exceeded, detailed per-call logging is omitted and one bounded overflow marker is emitted without changing request execution. Each retry starts at attempt 1 and has an eight-attempt logging cap; excess attempts remain governed by the existing retry policy and emit one bounded overflow marker rather than changing behavior. Keep normalized `source_ip` but omit `source_ip_forwarded`, while preserving trusted-proxy source resolution. Use a centralized deny-by-default field builder with an explicit per-tool field map, fixed types/counts/lengths, and no arbitrary `*_id` inference. Search logs omit raw search text and presence flags, and may include only bounded numeric parameters or allowlisted filter IDs. Trace IDs come only from a valid current `SpanContext`, are fixed-width correlation values rather than identity/authentication, and are omitted for absent/invalid/zero contexts. Tool logs emit explicit invocation and completion events; completion includes outcome, duration, and bounded result counts where available. Outbound InvenTree request logs use explicit `inventree_request_started`/`inventree_request_completed` events and the inherited correlation ID plus call sequence and attempt, making multi-call tools diagnosable without logging request bodies, credentials, raw query values, full URLs, or unbounded record data; they derive the request logger from context and add method, an operation and route-template selected from a checked-in closed registry whose complete inventory is the union of every included `internal/inventree.Client` method and its exact `docs/api-schema.yaml` endpoint-manifest entry, with no unlisted or runtime-derived routes. The registry has one exact entry per included method, records the finite request family (`json_api`, `multipart_api`, `attachment_download`, `image_download`, or `data_output_download`), and drift tests fail when a method or manifest endpoint is missing, duplicated, or unclassified. A shared request-classification marker/helper must identify every included request family, while excluding OAuth, bootstrap, MCP well-known metadata, and arbitrary URL-fetch traffic. Non-tool included calls use a stable `operation=system.<name>` field instead of `tool`; credential-bearing OAuth/bootstrap calls remain excluded. Completion outcomes use the closed vocabulary `success`, `validation_failure`, `authorization_failure`, `upstream_failure`, `cancellation`, and `internal_failure`; paginated/search result counts mean returned items only and are omitted when unavailable. Packaged tracing configuration remains disabled by default and mirrors the documented OTEL settings.
- Scope: update independent request/tool and outbound InvenTree API logging, centralized safe tool/API context extraction, completion/outcome/duration logging, trace correlation, injectable clock and isolated tracer seams, request classification, redaction tests, packaged YAML samples, and relevant README/operator/reference documentation.
- Acceptance:
  - Tool invocation logs are INFO-level and include tool, transport/request context, and normalized `source_ip` without `source_ip_forwarded`.
  - Structured tool logs remain active when OTEL tracing and metrics are both disabled.
  - Safe tool-specific context appears for ID-based tools; search and bulk contexts use bounded safe fields; raw queries, arbitrary arguments, credentials, tokens, uploaded content, and full records/results are never logged.
  - Completion logs include outcome and duration, plus a bounded result count where applicable; errors use safe classifications rather than sensitive upstream details.
  - Outbound InvenTree request logs emit both started and completed events and identify the originating tool or stable `operation=system.<name>`, method, finite fixed normalized API operation/route-template value, status or closed safe error class, duration, server-generated request ID, call sequence, and retry/attempt when applicable, allowing multi-call tool sequences to be reconstructed without request bodies, auth headers, raw query values, full URLs, or unbounded record data; stable request/tool/trace fields are inherited from the context-carried logger.
  - Valid fixed-width `trace_id`/`span_id` values are included only for a valid current span after MCP span creation, with no bogus IDs when tracing is absent, invalid, or zero; raw trace headers are never logged and trace IDs are not treated as identity or authorization.
  - Tests cover context-carried logger inheritance/enrichment, log level with telemetry disabled, direct/trusted/untrusted/malformed source-IP forwarding and STDIO behavior, the centralized field map and bounds, representative ID/search/bulk context, tool invocation/completion and outbound-request started/completed fields, two API calls in one invocation, separate invocations using the same route, an injectable request-ID generator with deterministic format/uniqueness/failure tests, bounded correlation reset/uniqueness, overflow markers without execution changes, route/error redaction, exact duration with an injectable clock, isolated tracer-provider behavior, and trace correlation for absent/zero/invalid/remote-parent/local-child spans.
  - `packaging/inventree-mcp.yml` includes the complete applicable HTTP configuration examples, including commented OTEL tracing settings matching `docs/examples/inventree-mcp.yml`, with safe guidance for collector headers and other secret-bearing configuration. A raw-template inventory test asserts these deterministic states: active HTTP examples (`transport`, `listen`, `path`, `environment`, `log_level`, `inventree_url`); commented HTTP examples (`inventree_web_url`, `inventree_timeout`, `inventree_tls_skip_verify`, `mcp_max_request_body_bytes`, `upload_max_bytes`, `debug_traffic_log`, `bulk_max_items`, `bulk_concurrency`, `otel_enabled`, `otel_service_name`, `otel_exporter`, `otel_endpoint`, `otel_insecure`, `otel_headers`, `otel_sample_ratio`, `otel_batch_timeout`, `otel_export_timeout`, `otel_metrics_enabled`, `otel_metrics_path`, `oauth_issuer_url`, `oauth_resource_url`, `oauth_keys`, `oauth_client_ids`, `oauth_access_lifetime`, `oauth_refresh_lifetime`, `oauth_session_lifetime`, `trusted_proxy_cidrs`, `bootstrap_enabled`, and `bootstrap_envelope_lifetime`); development-only commented (`dev_incomplete_oauth`); forbidden/inapplicable and absent as usable HTTP settings (`inventree_auth_scheme`, raw `inventree_token`, and STDIO-only `upload_allow_roots`). The `debug_traffic_log` example warns that it captures sensitive URIs, bodies, responses, and possible credentials, is owner-only (0600), requires bounded retention/deletion, and is for troubleshooting rather than routine production use. Every audited setting is classified as valid for production HTTP, documented but rejected outside development, or transport-specific and omitted; omissions are explicitly explained.
  - README, operator recipes, and relevant reference docs remain aligned; validation runs `GOFLAGS=-trimpath go test -race ./...`, `GOFLAGS=-trimpath INVENTREE_TEST_SKIP_DOCKER=1 go test -race ./...`, `go vet ./...`, `golangci-lint run ./...`, `go test ./internal/config ./packaging`, `goreleaser release --snapshot --clean`, and a documented per-package coverage comparison against `main`; the required GitHub checks `test`, `lint`, `Analyze (go)`, and `goreleaser-snapshot` plus the Go, QA, Product, and Infosec review panel complete with no unresolved findings. Gremlins is intentionally not required because F-S73 removed it from CI.
- Out of scope: changing OTEL exporter behavior or protocol, changing metrics, logging raw tool arguments/results, changing source-IP trust or rate-limiting semantics, or adding unbounded/high-cardinality fields.
- Tasks:
  - [ ] Define the allowlisted per-tool context fields and structured invocation/completion log contract, including context-carried logger ownership and enrichment boundaries.
  - [ ] Implement context-carried INFO invocation/completion logs, outbound InvenTree request completion logs, request classification, injectable request-ID/clock/tracer seams, and trace correlation.
  - [ ] Add focused tool/API context, completion, multi-call, trace, deterministic clock/tracer, route/error, and redaction tests; update documentation.
  - [ ] Audit and complete the packaged HTTP configuration examples, add the OTEL tracing sample, and validate the template against the real YAML config loader plus an explicit expected-field inventory that can detect missing commented examples.
  - [ ] Run the named default/race/vet/lint/package/snapshot/coverage validation and the applicable reviewer panel; record residual risk before marking the story Done.
- Residual risk: richer logs can become high-cardinality or disclose operator data if new tools bypass the allowlist; keep extraction centralized and protect representative tool categories with redaction tests.
