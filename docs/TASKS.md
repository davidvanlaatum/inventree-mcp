...
- Acceptance:
  ...
  - A route matrix covers default and non-default prefixes, normalization of `/mcp/` to `/mcp`, nearby paths such as `/mcpfoo`, empty/root prefixes, prefixes overlapping metrics or well-known metadata, the exact path-specific metadata locations derived from resource and issuer paths (including `/mcp` resource and origin-only issuer forms), arbitrary `/.well-known/*` negative cases, exact `/prefix` and `/prefix/*` proxy behavior, and confirms at both the MCP listener and Caddy/reverse-proxy boundary that root-level `/authorize` and `/token` return HTTP 404, do not reach the new handlers or InvenTree, and have no compatibility alias.
...
- Acceptance:
  ...
  - `packaging/inventree-mcp.yml` includes the complete applicable HTTP configuration examples, including commented OTEL tracing settings matching `docs/examples/inventree-mcp.yml`, with safe guidance for collector headers and other secret-bearing configuration. A raw-template inventory test asserts the expected active/commented/forbidden state for this explicit audit list: `transport`, `listen`, `path`, `environment`, `log_level`, `inventree_url`, `inventree_web_url`, `inventree_timeout`, `inventree_tls_skip_verify`, `mcp_max_request_body_bytes`, `upload_max_bytes`, `debug_traffic_log`, `bulk_max_items`, `bulk_concurrency`, `otel_enabled`, `otel_service_name`, `otel_exporter`, `otel_endpoint`, `otel_insecure`, `otel_headers`, `otel_sample_ratio`, `otel_batch_timeout`, `otel_export_timeout`, `otel_metrics_enabled`, `otel_metrics_path`, `oauth_issuer_url`, `oauth_resource_url`, `oauth_keys`, `oauth_client_ids`, `trusted_proxy_cidrs`, `bootstrap_enabled`, `bootstrap_envelope_lifetime`, and the development-only `dev_incomplete_oauth`; it also asserts that HTTP-inapplicable `inventree_auth_scheme`, raw `inventree_token`, and STDIO-only `upload_allow_roots` are not presented as usable HTTP settings. The `debug_traffic_log` example warns that it captures sensitive URIs, bodies, responses, and possible credentials, is owner-only (0600), requires bounded retention/deletion, and is for troubleshooting rather than routine production use. Every audited setting is classified as valid for production HTTP, documented but rejected outside development, or transport-specific and omitted; omissions are explicitly explained.
