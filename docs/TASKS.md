...
- Progress: implementing on `claude/f-s90-9cbef6`. Operator-approved decisions: single generic `render_component_image` tool with a `family` discriminator (not five per-family tools); rendering uses a new `github.com/fogleman/gg` (MIT) pure-Go 2D graphics dependency plus `golang.org/x/image/font/basicfont` for markings text (no embedded font asset, avoids a font-license question); resistor color-band encoding covers 4-band and 5-band (6-band temperature-coefficient encoding deferred as a documented future extension); the tool requires `inventree.read` even though it makes no InvenTree API calls, reusing the existing OAuth authenticate-and-scope-check path so it is not exposed unauthenticated over HTTP (operator explicitly asked for "no [InvenTree] scope" not to mean "no auth").
...
- Validation: `go build ./...`, `go vet ./...`, `gofmt -l .` (clean), `golangci-lint run ./internal/render/... ./internal/tools/...` (0 issues), `go mod tidy` (moved `github.com/fogleman/gg` to the direct-require block per Go review), `go test ./internal/render/... -cover` (97.1%), `go test ./internal/tools/...` (86.1%, baseline on `main` was 86.0%, no package-level regression), `go test ./... -race` (all packages pass). `docs/api-schema.md` is unchanged: this story makes zero InvenTree API calls, so there is no endpoint/capability surface to record there.
- Review: Senior Go Developer, Senior QA / Test Architect, and Senior Product Manager subagent review completed (minimum panel for a Go + test + operator-facing-tool change per `docs/reviewers.md`; the full four-role panel was not run because this story adds no OAuth/HTTP-auth behavior, no new mutating tool, and no staff-scoped/administrative read — it is a local-only, read-scoped rendering tool). Go: no correctness issues in the orientation/rotation math, the IEC 60062 resistor value-to-band decomposition, determinism (no map-iteration-order dependence in any drawing path), or the scope-without-API-call wiring through the existing `ToolAuthorizations`/`GuardTool` mechanism; one hygiene finding (`github.com/fogleman/gg` listed indirect instead of direct in `go.mod`) fixed via `go mod tidy`. QA: flagged six coverage gaps — no test proved the renderer was actually sensitive to its inputs (only that identical inputs matched), no test exercised `MaxOutputBytes`, no test hit the `MinDimensionPx`/`MaxDimensionPx` boundaries inclusively, no test verified the vertical-orientation rotation's exact direction/axis mapping (only final width/height), and no test covered mixed-case hex color rejection or the markings-length boundary at exactly 16 characters — all six addressed with new tests in `internal/render/render_test.go`, `diode_test.go`, and `capacitor_test.go`; confirmed the deliberate absence of Testcontainers coverage is correct since the tool never calls InvenTree. Product: scope/acceptance/out-of-scope stayed consistent across `docs/TASKS.md`, issue #235, and all four docs surfaces; flagged that the "don't approximate an unsupported family" guardrail existed only in `docs/operator-recipes.md` (never seen by a calling agent at runtime) — addressed by adding the same guardrail to the tool's own `family` field description and tool description in `internal/tools/component_render_tools.go`; separately noted capacitor's `diameter_mm`/`height_mm` fields drop the `body_` prefix used by resistor/diode's `body_length_mm`/`body_diameter_mm`, flagging a possible silent-field-drop risk if an agent guessed the wrong name — verified this is a non-issue because the MCP SDK's generated JSON schema sets `additionalProperties:false` on every struct-based tool input (confirmed by dumping `RenderCapacitorInput`'s generated schema), so a wrong field name is a clear schema validation rejection, not silent data loss; no code change made for this finding, left as documented residual naming note below.
- Residual risk: package-variant `size` presets and derived resistor color bands are illustrative only; a caller that wants true-to-scale imagery must supply the optional paired dimension fields, and even then the renderer does not claim absolute canvas-to-millimeter accuracy. Capacitor's `diameter_mm`/`height_mm` field names intentionally drop the `body_` prefix used by the two axial families (radial vs. axial geometry differ); an agent guessing the axial vocabulary for capacitor gets a fail-closed schema rejection rather than a silently ignored field, so this is a minor discoverability rough edge, not a correctness risk. 6-band (temperature-coefficient) resistor coverage remains a documented future extension, not implemented here.
