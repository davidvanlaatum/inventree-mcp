...
- Decisions: do not store a per-user token mapping on the MCP server. The server returns an opaque, encrypted and authenticated MCP bearer envelope containing a dedicated InvenTree API credential. The normal `/mcp` endpoint accepts only that MCP-owned bearer envelope, never a raw inbound InvenTree `Token` or `Bearer` credential. The bootstrap operation must be a separate endpoint, use HTTP Basic authentication only over the configured canonical HTTPS origin, avoid logging credentials, and apply bounded request/rate-limit protections. The exact Basic-auth credential convention remains an implementation decision: current InvenTree integration validates an existing API credential and creates a dedicated token through `/api/user/me/token/`; it does not provide a generic username/password login operation.
- Scope: add a production-safe non-OAuth bootstrap flow for a user to exchange an InvenTree credential presented with Basic auth for a stateless MCP bearer token. The returned token is entered into the remote MCP client's static bearer-token configuration. Each user therefore retains their own InvenTree permissions and credential identity without a shared token or server-side mapping database.
- Acceptance:
  - Add an explicitly configured HTTP authentication mode for stateless bearer bootstrap, with clear interaction rules for OAuth mode and a fail-closed production configuration.
  - Add a dedicated bootstrap endpoint that requires the configured canonical HTTPS deployment, parses bounded Basic auth, validates the supplied credential against the configured InvenTree instance, creates a dedicated InvenTree API token using the existing broker/client capability, and returns only an opaque MCP token.
  ...
  - Update `docs/PLAN.md`, `docs/operator-recipes.md`, README configuration guidance, packaged examples, and relevant auth/config reference docs with the endpoint, client setup, security model, and open credential-convention decision.
...
- Open decisions:
  - Choose and document the Basic-auth convention. The likely initial contract is Basic username plus an InvenTree API token as the password, because that matches the currently supported upstream credential model; accepting an InvenTree username/password would require a separately verified upstream login/token-generation contract.
  - Decide whether bootstrap and OAuth may coexist on the same deployment or must be mutually exclusive, including the configuration and proxy paths for each.
- Tasks:
