...
- Status: `Done`
- Issue: [#137](https://github.com/davidvanlaatum/inventree-mcp/issues/137) (status-label update to `status:done` and assignment blocked by a GitHub integration permission error; see Progress)
...
- Progress: implementation started on `claude/f-s56-cross-object-tags` from `origin/main` at `7054660` on 2026-08-28. Attempting to update issue #137's status label/assignment through the GitHub MCP integration failed with `403 Resource not accessible by integration`; per this repo's status-label policy the transition was stopped rather than worked around, and `docs/TASKS.md` (this file) is the authoritative status record until the label is reconciled manually or the integration's permissions are widened. Added a `cross_object_tag_workflow_discovery` subtest to `internal/inventree/client_methods_integration_test.go` and ran it live against pinned InvenTree `1.5.2`.
