...
- Decisions: OAuth authorization and token endpoints will be exposed at `<configured-prefix>/oauth/authorize` and `<configured-prefix>/oauth/token`; with the default prefix this is `/mcp/oauth/authorize` and `/mcp/oauth/token`. The configured OAuth issuer URL remains the canonical origin used for metadata and token validation, while its advertised endpoint URLs include the configured prefix. Protected-resource and authorization-server `/.well-known/...` metadata locations remain unchanged. The reverse proxy must route the configured prefix and its subpaths to MCP, while ordinary InvenTree paths remain separate. This is an intentional endpoint migration: the new server returns no OAuth handler at root-level `/authorize` or `/token`, so clients must rediscover metadata or be reconfigured; during rollout, any temporary old-route proxying is only to an older MCP deployment and must not route those paths to InvenTree.
...
- Acceptance:
  ...
  - A route matrix covers default and non-default prefixes, trailing-slash normalization, nearby paths such as `/mcpfoo`, metrics collisions, metadata paths, exact `/prefix` and `/prefix/*` proxy behavior, and confirms root-level `/authorize` and `/token` do not reach the new handlers.
...
- Residual risk: clients or reverse proxies using the old root-level OAuth endpoint URLs will need to rediscover metadata or be reconfigured after deployment; the old routes must remain proxied until the new release is installed everywhere that uses OAuth.
