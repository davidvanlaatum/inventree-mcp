...
`render_and_attach_component_image` (F-S105) is a second, separately registered tool sharing `render_component_image`'s exact family/parameter contract, added because relaying a rendered image's `base64` bytes from `render_component_image` into a follow-up `upload_attachment`/`set_primary_image` call proved unreliable for a calling model doing this repeatedly across many parts. It adds a mandatory `attach` object (`part_id`, optional `set_primary`, and `confirm` when replacing an existing primary image) and, on the server side only, uploads the rendered bytes as a part attachment and, when requested, sets it as the part's primary image using `SetPartPrimaryImage`'s existing confirm-gated replacement semantics — the caller only ever relays a `part_id`, never the image bytes. The attachment's filename is content-addressed (the component family plus a prefix of the rendered PNG's own SHA-256 digest), so a repeated call with identical parameters against the same part reuses the existing attachment rather than uploading a duplicate, without needing an `allow_duplicate` field the way `upload_attachment` has. If the upload succeeds but the subsequent primary-image set fails, the tool returns `status:partial_failure` with a `recovery_plan` naming the created `attachment_id` (and, when that failure happened while replacing an existing primary image, noting the retry needs `confirm:true` again) rather than retrying automatically. This is a distinct tool rather than an optional field on `render_component_image` because `render_component_image` is always registered, even when this server's write tools are disabled, and this repo's tool-authorization contract requires a tool's declared mutation class and scopes to reflect what it can actually do; `render_and_attach_component_image` is registered only alongside this server's other write tools and requires `inventree.read`, `inventree.write`, and `inventree.upload`. `render_component_image` itself is unchanged by this story.
...
1. The bootstrap request carries exactly one credential, as the standard `Authorization` header on the POST itself: `Basic <base64 user:pass>` or `Token`/`Bearer <token>`. There is no separate JSON credential-form body and therefore no request-form precedence to resolve.
2. `inventree.AuthSchemeBasic` exists only for this transient outbound validation call. `oauth.Credential.ValidateForEnvelope` (used by every envelope open/seal path) rejects it, so a Basic credential can never be sealed into a long-lived MCP token envelope even by accident; InvenTree's OpenAPI schema (`docs/api-schema.yaml` `components.securitySchemes.basicAuth`) confirms `Authorization: Basic ...` already works against the same endpoints as Token auth, so no separate InvenTree login endpoint is needed.
3. On success, the existing `InvenTreeCredentialBroker` mints a fresh, uniquely named dedicated InvenTree token (`inventree-mcp-static-<random>`) the same way the OAuth setup flow does. InvenTree's token-issuance endpoint does not actually error on a name collision — it deletes and recreates a same-named token — so the "don't rotate an existing token" guarantee comes from the 128-bit random name alone; the bounded retry only helps a transient upstream failure and stops immediately once the request's context is done. If dedicated-token creation fails, the request fails closed: no envelope is issued, and the supplied credential is never sealed as a fallback — the handler independently re-validates the broker's returned credential with `ValidateForEnvelope` before sealing, so this guarantee does not rest solely on the broker implementation's convention.
4. The dedicated credential is sealed with a new `AssociatedData.Type` (`bootstrap_access`) and an empty `ClientID`, reusing `TokenClaims` unchanged (no new claims shape, no refresh token — renewal is re-bootstrap only). AES-GCM authenticates the AAD, so a bootstrap-typed envelope and a normal OAuth access-token envelope are cryptographically unforgeable as each other; they cannot be replayed as one another regardless of claims validation.
5. `AccessTokenVerifier` tries the existing per-client-ID OAuth AAD loop first, then — only when bootstrap is enabled — one additional `Open` attempt against the bootstrap AAD. Everything below the verifier (`CredentialFromContext`, `OAuthClientFromContext`, tool dependency injection) is unchanged; a bootstrap-issued envelope authorizes `/mcp` exactly like an OAuth access token.
6. Bootstrap envelopes carry the full scope set (`supportedOAuthScopes()`): there is no consent screen for this flow, so the user's own InvenTree permissions remain the real authorization boundary, matching the trust model of a directly issued InvenTree API token.
...
- InvenTree blocking integration-test baseline: InvenTree `1.5.4`, which reports API version `530` and passes the blocking integration suite against the existing client contracts. This pin does not define the minimum supported InvenTree version.
- Docker image for blocking Testcontainers tests: `inventree/inventree:1.5.4`, targeting the checked-in `docs/api-schema.yaml` OpenAPI 3.0.3 / API version `530` client contract. `internal/testenv/Dockerfile` exposes the same image pin to Dependabot and must match `internal/buildinfo.PinnedInvenTreeVersion`. Do not use a digest as the primary pin because the version should be clear in config, logs, and failure output. Do not use floating tags such as `stable` for blocking tests.
...
- API schema source baseline: current local `docs/api-schema.yaml` is the official `inventree/schema` API 530 export associated with an InvenTree source commit before the 1.5.0 release tag. Blocking Testcontainers coverage confirms that the released InvenTree `1.5.4` image still reports API `530` and satisfies the exercised client contracts. Provenance records both source identities; the pin does not establish a minimum supported InvenTree version.
...
- Upstream InvenTree auth schemes: `Token` and `Bearer` for any credential that gets sealed into a long-lived MCP token envelope or used for ordinary API calls. `Basic` exists only as a transient input form to the F-S93 stateless bearer bootstrap endpoint's credential-validation call; it is never sealed into an envelope and never appears as a scheme on an ordinary InvenTree API request.
...
- F-S55 through F-S60 are completed discovery stories for barcode, tags, testing, pricing, requirements, and stocktake generation/reporting. Approved implementation work is isolated into follow-ups F-S98 through F-S101 and F-S104 where applicable; rejected PO-duplication and stock-merge workflows remain explicitly out of scope.
- F-S61 is complete on `main` and establishes the InvenTree 1.5/API 530 baseline. The post-merge optional-field audit uses these explicit routes:
  - F-S40 owns part `consumable` and notes; nested category/default-location detail and `category_path` remain separate exact lookups, existing part parameters remain under F-S12, tags were added by F-S91, and price breaks and the computed pricing snapshot were added by F-S98.
  - F-S43 owns complete approved supplier/manufacturer-part exact detail, supplier availability/read metadata, and distinct long Markdown notes while keeping searches concise. Supplier `available` and both long-note fields are ordinary verified writes; computed supplier availability timestamps, in-stock, and on-order values remain read-only. Embedded records stay separate lookups, parameters remain in their dedicated workflows, price breaks were added by F-S98, and tags were added by F-S91, raw `barcode_hash` remains excluded (SupplierPart/ManufacturerPart are out of scope for F-S99, which covers only Part, StockItem, StockLocation, and PurchaseOrder), and API 530 `duplicate` inputs are write-only commands excluded pending separately approved guarded duplication workflows. F-S44 owns complete company exact reads (phone, email, contact, tax ID, external `link`, primary-image URL, notes, supplied/manufactured counts, customer role) and their guarded writes under the same duplication boundary; `primary_address` remains a separate structured-address lookup, parameters remain F-S64, and tags were added by F-S91.
  - F-S45 owns stock-item SKU, MPN, `expired`, `stale`, `sales_order_reference`, and `location_path` on the separate exact `get_stock_item` projection while `search_stock_items` stays concise; nested stock location/part/supplier-part detail remains separate, stock tags were added by F-S91 (`get_stock_item` only; no update tool), tests remain F-S57, and raw `barcode_hash` remains excluded by design (F-S99 derives a computed `has_barcode` bool from it instead). F-S46 projects new tracking detail through stable IDs and safe display fields rather than embedded full records.
  - F-S47 owns purchase-order notes, line/extra-line discount, and nullable total-price reads; supplier/order/part detail remains separate, tags were added by F-S91, and PO duplication is explicitly rejected as an MCP workflow; normal guarded PO creation remains the reorder path.
  ...
  - F-S56 investigated all newly schema-visible tags and its implementation follow-up F-S91 added them across the seven covered object types; F-S58 investigated price-break/pricing workflows and its implementation follow-up F-S98 added read and guarded write tools for internal/sale/supplier price breaks and the computed part-pricing snapshot. Pinned inventories must classify null, omission, read-only, write-only, embedded-detail, and deferred behavior so future API drift cannot silently widen a tool.
  No additional story is required for the API 530 additions. F-S62 through F-S69 extend the operational inventory focus with purchase-order hold/resume/cancel and metadata maintenance, rejected PO duplication, cross-object parameters, stock custom status, rejected stock merge, location detail/type administration, and guarded location/category deletion.
...
- Blocking integration tests target InvenTree `1.5.4`, which reports API version `530` and passes the exercised client contracts from the checked-in schema baseline. This test pin does not define the minimum supported InvenTree version.
