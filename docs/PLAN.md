...
- F-S61 is complete on `main` and establishes the InvenTree 1.5/API 530 baseline. The post-merge optional-field audit uses these explicit routes:
  ...
  - F-S43 owns sourcing availability/read metadata and long notes; F-S44 owns company notes and ordinary metadata; their API 530 `duplicate` inputs are write-only commands excluded pending separately approved guarded duplication workflows.
