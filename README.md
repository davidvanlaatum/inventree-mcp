...
| `v0.0.15`-`v0.1.1` | 1.5.2 | 530 |
| `v0.1.2`-`v0.2.0` | 1.5.4 | 530 |
| `main` (unreleased) | 1.5.5 | 530 | <!-- inventree-compat-table:current-row -->
...
- OpenTelemetry tracing is disabled by default. Enable it with `--otel-enabled` or `INVENTREE_MCP_OTEL_ENABLED=true`, then configure `--otel-exporter otlpgrpc|otlphttp`, `--otel-endpoint`, optional repeated `--otel-header key=value`, `--otel-insecure` for a trusted local non-TLS collector, `--otel-sample-ratio`, `--otel-batch-timeout`, and `--otel-export-timeout`. Equivalent YAML keys are `otel_enabled`, `otel_exporter`, `otel_endpoint`, `otel_headers`, `otel_insecure`, `otel_sample_ratio`, `otel_batch_timeout`, and `otel_export_timeout`. Use a `host:port` endpoint for either exporter or a full `http(s)://...` URL for OTLP/HTTP; header values may contain exporter credentials, so protect the YAML file and process environment accordingly. Tracing carries W3C trace context through MCP methods and outbound InvenTree/OAuth HTTP requests without recording arguments or response payloads; numeric top-level tool identifiers are recorded only when supplied under an `id` or `*_id` field. Inbound HTTP spans start with bounded route-template names such as `<method> /mcp` and MCP tool spans use `mcp.<method>/<registered-tool>`; for HTTP calls, the root span is renamed to that bounded tool operation so tracing list views identify the tool directly. Unknown methods and tools use `other`.
- Optional Prometheus-compatible OpenTelemetry metrics require HTTP mode. Enable them with `--otel-metrics-enabled` or `INVENTREE_MCP_OTEL_METRICS_ENABLED=true`; the scrape endpoint defaults to `/metrics` and can be changed with `--otel-metrics-path`, `INVENTREE_MCP_OTEL_METRICS_PATH`, or YAML keys `otel_metrics_enabled` and `otel_metrics_path`. The endpoint is intentionally unauthenticated by MCP OAuth: production requires a private loopback/RFC-private listener; a reverse proxy may additionally enforce a scraper allowlist. Metrics include MCP requests, per-tool call counts/durations/in-flight gauges, outbound HTTP requests/durations, allowlisted InvenTree API resource counts/durations/in-flight gauges, and bulk operations started. Tool and API labels are fixed registered/allowlisted names (`other` for unknown values); credentials, payloads, record IDs, and upstream URLs are not exported.
...
- Optional `INVENTREE_MCP_BOOTSTRAP_ENABLED` and `INVENTREE_MCP_BOOTSTRAP_ENVELOPE_LIFETIME` — see below.

All of the above OAuth settings are required for production HTTP mode even if a deployment intends to use only the bootstrap flow below: OAuth registration is not yet optional in this release, so `INVENTREE_MCP_OAUTH_CLIENT_IDS` and the OAuth lifetimes must still be configured.
...
For remote HTTP clients that only support a single statically configured bearer token and cannot perform the OAuth flow above, set `INVENTREE_MCP_BOOTSTRAP_ENABLED=true` (or `--bootstrap-enabled`) to additionally enable a coexisting stateless bearer bootstrap endpoint at `<INVENTREE_MCP_PATH>/auth/bootstrap`, on top of the OAuth configuration above (still required). Each user exchanges their own InvenTree Basic (username/password) or Token/Bearer credential, supplied as the bootstrap request's own `Authorization` header, for an opaque MCP bearer envelope containing a freshly minted `inventree-mcp-static-*` dedicated token; that envelope is pasted into the client's static bearer configuration. Envelopes expire after `INVENTREE_MCP_BOOTSTRAP_ENVELOPE_LIFETIME` (default 720h / 30 days) with re-bootstrap as the only renewal path — renewal therefore accrues a new dedicated InvenTree token each cycle, and `bootstrap_enabled: false` plus a restart immediately revokes every outstanding bootstrap envelope without affecting OAuth sessions. See [Stateless Bearer Bootstrap Setup](docs/operator-recipes.md#stateless-bearer-bootstrap-setup) for the full request/response shape, fail-closed behavior, and lifecycle trade-offs. As with the OAuth connector setup above, live packaged deployment validation for this flow remains F-S10 work.

Production supports path-preserving reverse proxies only. `INVENTREE_MCP_PATH` must exactly match the path in `INVENTREE_MCP_OAUTH_RESOURCE_URL`, including any public prefix, and the proxy must forward that path unchanged. Canonical OAuth URLs always come from the configured issuer/resource URLs; `X-Forwarded-Host`, `X-Forwarded-Proto`, and `X-Forwarded-Prefix` are ignored. `X-Forwarded-For` is used only when the immediate peer is in `INVENTREE_MCP_TRUSTED_PROXY_CIDRS`; the normalized client address is then used for rate limiting and request-scoped logging without recording the raw forwarding header. The OAuth authorization and token endpoints are exposed under that same `INVENTREE_MCP_PATH` prefix (`<path>/oauth/authorize`, `<path>/oauth/token`, for example `/mcp/oauth/authorize`) rather than at the issuer URL's own root, so the proxy only ever needs to route one MCP-owned prefix; the reverse proxy must not route the old root-level `/authorize` or `/token` paths to this server.
...
The packaged service is intended for HTTP mode behind a path-preserving reverse proxy. Production HTTP startup validates OAuth envelope keys, issuer/resource URLs, exact public/internal MCP path alignment, trusted proxy CIDRs, allowed client IDs, and token lifetimes before serving protected MCP traffic and connector authorization routes. The systemd unit uses `Type=notify`, reports ready only after those checks pass and the HTTP listener is bound, sends stdout/stderr explicitly to journald with the `inventree-mcp` identifier, and sends watchdog heartbeats every half of systemd's configured 30-second watchdog interval. Once the managed HTTP lifecycle is initialized, graceful shutdown and fatal runtime failures publish sanitized service status. Configuration or logger initialization failures that occur before that lifecycle starts still exit non-zero, allowing systemd to record the failure and apply `Restart=on-failure` without a separate status notification path. If a heartbeat cannot be delivered, the server reports a degraded status and keeps serving; systemd's watchdog timeout terminates it and starts a replacement. Install packages now for file layout testing, but do not enable the systemd service for a live ChatGPT connector until F-S10 live packaged deployment validation lands.

The packaged unit runs as a dedicated non-root `inventree-mcp` system user/group instead of root (F-S87). `packaging/scripts/preinstall.sh` creates the system user/group (via `useradd`/`groupadd` or, on Alpine, `adduser`/`addgroup`) before package files are unpacked, and `postinstall.sh` fixes `/etc/inventree-mcp/config.yml` ownership to `inventree-mcp:inventree-mcp` on every install and upgrade. `postremove.sh` removes the system user/group only on final package removal (`dpkg purge`, the final `rpm` erase, or any `apk` removal), not on a version upgrade. A package-created static system user was chosen over `DynamicUser=yes` because a dynamically allocated identity is not guaranteed to persist across reboots without an otherwise-unneeded `StateDirectory=`/`CacheDirectory=`/`LogsDirectory=`, which would risk `config.yml` ownership drifting after a reboot.

Edit `/etc/inventree-mcp/config.yml` (mode `0600`, owned by `inventree-mcp:inventree-mcp`) to set the InvenTree URL, OAuth key material, and any other packaged HTTP-mode settings; the packaged unit runs `inventree-mcp serve --config /etc/inventree-mcp/config.yml` as that user.
...
- Dependabot version updates are enabled by `.github/dependabot.yml` for Go modules, GitHub Actions, pre-commit hooks, and the Dockerfile that exposes the blocking Testcontainers InvenTree baseline.
