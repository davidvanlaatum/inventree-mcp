...
For remote HTTP clients that only support a single statically configured bearer token and cannot perform the OAuth flow above, set `INVENTREE_MCP_BOOTSTRAP_ENABLED=true` (or `--bootstrap-enabled`) to enable a coexisting stateless bearer bootstrap endpoint at `<INVENTREE_MCP_PATH>/auth/bootstrap`. A user exchanges an InvenTree Basic (username/password) or Token/Bearer credential, supplied as the bootstrap request's own `Authorization` header, for an opaque MCP bearer envelope containing a freshly minted `inventree-mcp-static-*` dedicated token; that envelope is pasted into the client's static bearer configuration. Envelopes expire after `INVENTREE_MCP_BOOTSTRAP_ENVELOPE_LIFETIME` (default 720h / 30 days) with re-bootstrap as the only renewal path. See [Stateless Bearer Bootstrap Setup](docs/operator-recipes.md#stateless-bearer-bootstrap-setup) for the full request/response shape, fail-closed behavior, and lifecycle trade-offs.
